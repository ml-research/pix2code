...
def randomWord(minimum=1, predicate=None):
    ...
    if WORDS is None:
        ...
        observations = {
            "".join(z) for t in tasks for xs, y in t.examples for z in list(xs) + [y]
        }
...
def makeTasks():
    ...
    for d1, d2 in randomPermutation(crossProduct(delimiters, delimiters))[
        ...
        problem(
            "Append two words delimited by '%s%s'" % (d1, d2),
            [
                ((x, y), x + d1 + d2 + y)
                for _ in range(NUMBEROFEXAMPLES)
                for x in [randomWord()]
    ...
    for d1, d2 in randomPermutation(crossProduct(delimiters, delimiters))[
        ...
        problem(
            "Extract word delimited by '%s' - '%s'" % (d1, d2),
            [
                ((a + d1 + b + d2 + c + d + e,), b)
    ...
    for d1, d2 in randomPermutation(crossProduct(allowed, allowed))[: len(delimiters)]:
        problem(
            "parentheses around word delimited by '%s' & '%s'" % (d1, d2),
            [
                (
...
def loadPBETasks(directory="PBE_Strings_Track"):
    ...
    for f in os.listdir(directory):
        ...
        task = Task(
            name,
            arrow(*[tstr] * (len(examples[0][0]) + 1)),
            [(tuple(map(explode, xs)), explode(y)) for xs, y in examples],
        )
        cheat = task
...
def guessConstantStrings(task):
    if task.request.returns() == tlist(tcharacter):
        ...
        task.stringConstants = [g for g, f in guesses.items() if f >= T]
    else:
    ...
    task.specialTask = (
        "stringConstant",
        {"maxParameters": task.maxParameters, "stringConstants": task.stringConstants},
    )
