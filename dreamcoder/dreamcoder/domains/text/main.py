...
from dreamcoder.domains.text.makeTextTasks import loadPBETasks, makeTasks
    ...
    class LearnedFeatureExtractor(RecurrentFeatureExtractor):
        ...
        def __init__(self, tasks, testingTasks=[], cuda=False):
            ...
            super(LearnedFeatureExtractor, self).__init__(
                lexicon=list(lexicon), H=64, tasks=tasks, bidirectional=True, cuda=cuda
            )
            self.MAXINPUTS = 8
...
def competeOnOneTask(checkpoint, task, CPUs=8, timeout=3600, evaluationTimeout=0.0005):
    if checkpoint.recognitionModel is not None:
        recognizer = checkpoint.recognitionModel
        challengeFrontiers, times, bestSearchTime = recognizer.enumerateFrontiers(
            [task],
            CPUs=CPUs,
            maximumFrontier=1,
        ...
        challengeFrontiers, times, bestSearchTimes = multicoreEnumeration(
            checkpoint.grammars[-1],
            [task],
            CPUs=CPUs,
            maximumFrontier=1,
            enumerationTimeout=timeout,
...
def sygusCompetition(checkpoints, tasks):
    ...
    for t in tasks:
        for checkpoint in checkpoints:
            promise = workers.apply_async(
                competeOnOneTask, (checkpoint, t), {"CPUs": CPUs, "timeout": timeout}
            )
    ...
    searchTimes = {t: min(ts) if len(ts) > 0 else None for t, ts in searchTimes.items()}
...
def main(arguments):
    ...
    challenge, challengeCheating = loadPBETasks(
        "data/sygus" if latest else "PBE_Strings_Track"
    )
