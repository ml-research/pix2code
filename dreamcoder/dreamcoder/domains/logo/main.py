...
from dreamcoder.domains.logo.logoPrimitives import (primitives, tangle,
                                                    tlength, turtle)
from dreamcoder.domains.logo.makeLogoTasks import (drawLogo, makeTasks,
                                                   montageTasks)
...
try:
    from dreamcoder.recognition import maybe_cuda, variable
except:
    print(
...
from dreamcoder.utilities import eprint, loadPickle, testTrainSplit
...
try:
    ...
    class LogoFeatureCNN(nn.Module):
        ...
        def forward(self, v):
            ...
            reshaped = [
                floatOnlyTask[i : i + self.inputImageDimension]
            ...
            v = torch.unsqueeze(v, 0)
            v = maybe_cuda(v, next(self.parameters()).is_cuda) / 256.0
...
def list_options(parser):
    ...
    parser.add_argument("--animate", default=None, type=str)
...
def enumerateDreams(checkpoint, directory):
    ...
    g = result.grammars[-1]
    if directory is None:
        assert False, "please specify a directory"
    eprint(" Dreaming into", directory)
    os.system("mkdir  -p %s" % directory)
    ...
    while len(frontiers) > 0:
        ...
        if fs:
            ...
            dreamFromGrammar(
                [list(f.entries)[0].program for f in fs], f"{directory}/{MDL}"
            )
        MDL += 1
...
def visualizePrimitives(primitives, export="/tmp/logo_primitives.png"):
    ...
    from dreamcoder.program import Abstraction, Application, Index
    from dreamcoder.type import tint
    from dreamcoder.utilities import makeNiceArray, montageMatrix
    ...
    for p in primitives:
        ...
        for arguments in entireArguments.get(
            str(p), product(*[argumentChoices(t) for t in t.functionArguments()])
        ):
            eprint(arguments)
            pp = p
            for a in arguments:
...
def main(args):
    ...
    needsExport = [str(z) for _, _, z in r.grammars[-1].productions if z.isInvented]
    if save is not None:
