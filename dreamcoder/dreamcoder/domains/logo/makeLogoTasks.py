...
from dreamcoder.utilities import eprint, jsonBinaryInvoke, montage, random_seed
...
def drawLogo(
    *programs,
    ...
    if smoothPretty:
        message["smoothPretty"] = smoothPretty
    if timeout:
    ...
    if isinstance(resolution, list):
        assert len(resolution) == len(
            programs
        ), "must provide a resolution for each program"
    ...
    if cost:
        ...
        response = [
            programResponse
            if isinstance(programResponse, str)
            else (programResponse["pixels"], programResponse["cost"])
...
def parseLogo(s):
    ...
    def command(k, environment, continuation):
        ...
        if k[0] == Symbol("move"):
            return Application(
                Application(
                    Application(_move, expression(k[1], environment)),
        ...
        if k[0] == Symbol("for") or k[0] == Symbol("loop"):
            ...
            return Application(
                Application(Application(_loop, b), Abstraction(Abstraction(body))),
                continuation,
            )
        if k[0] == Symbol("embed"):
...
def dSLDemo():
    ...
    def T(source):
        demos.append(
            manualLogoTask(str(len(demos)), source, lambdaCalculus="lambda" in source)
        )

    # this looks like polygons - verify and include
    T(
        "(#(lambda (lambda (#(lambda (lambda (#(lambda (lambda (lambda (logo_forLoop $0 (lambda (lambda (logo_FWRT $4 $3 $0))))))) $1 $0 logo_IFTY))) $1 (logo_DIVA logo_UA $0)))) (logo_MULL logo_UL 4) 3)"
    )
    T(
        "(#(lambda (lambda (#(lambda (lambda (#(lambda (lambda (lambda (logo_forLoop $0 (lambda (lambda (logo_FWRT $4 $3 $0))))))) $1 $0 logo_IFTY))) $1 (logo_DIVA logo_UA $0)))) (logo_MULL logo_UL 6) 4)"
    )
    T(
        "(#(lambda (lambda (#(lambda (lambda (#(lambda (lambda (lambda (logo_forLoop $0 (lambda (lambda (logo_FWRT $4 $3 $0))))))) $1 $0 logo_IFTY))) $1 (logo_DIVA logo_UA $0)))) (logo_MULL logo_UL 5) 5)"
    )
    T(
        "(#(lambda (lambda (#(lambda (lambda (#(lambda (lambda (lambda (logo_forLoop $0 (lambda (lambda (logo_FWRT $4 $3 $0))))))) $1 $0 logo_IFTY))) $1 (logo_DIVA logo_UA $0)))) (logo_MULL logo_UL 3) 6)"
    )
    T(
        "(#(lambda (lambda (#(lambda (lambda (#(lambda (lambda (lambda (logo_forLoop $0 (lambda (lambda (logo_FWRT $4 $3 $0))))))) $1 $0 logo_IFTY))) $1 (logo_DIVA logo_UA $0)))) (logo_MULL logo_UL 2) 7)"
    )

    # Spirals!
    for spiralSize in [1, 2, 3, 4, 5]:
        T(
            f"((lambda (logo_forLoop logo_IFTY (lambda (lambda (logo_FWRT (logo_MULL logo_epsL $1) (logo_MULA logo_epsA $2) $0))))) {spiralSize})"
        )
    for spiralSize in [5, 6, 7, 8, 9]:
        ...
        T(
            "(loop i " + str(spiralSize) + " (move (*d 1l i) (/a 1a 4)))"
        )  # (#(lambda (logo_forLoop $0 (lambda (lambda (#(lambda (logo_FWRT (logo_MULL logo_UL $0) (logo_DIVA logo_UA 4))) $1 $0))))) {spiralSize} $0))")
    ...
    for circleSize in [1, 3, 5, 7, 9]:
        T(
            f"(lambda (#(lambda (logo_forLoop 6 (lambda (lambda (#(lambda (lambda (logo_forLoop logo_IFTY (lambda (lambda (logo_FWRT $2 $3 $0)))))) logo_epsA (logo_MULL logo_epsL $2) $0))))) {circleSize} $0))"
        )
...
def rotationalSymmetryDemo():
    ...
    body = {
        "dashed": "(p (move 1d 0a)) (move 1d 0a) (p (move 1d 0a)) (move 1d 0a)",
...
def manualLogoTasks():
    ...
    def T(name, source, needToTrain=False, supervise=False):
        tasks.append(
            manualLogoTask(name, source, supervise=supervise, needToTrain=needToTrain)
        )

    if False:
        for d, a, s in [
            ("1l", "0a", "(loop i infinity (move epsilonLength epsilonAngle))"),
            (
                "epsilonLength",
                "0a",
    ...
    for n, l, s in [
        ...
        T(
            f"{n}-gon {l}{'' if s is None else ' slanted '+str(s)}",
            f"""
    ...
    T("upwards", "((move 0d (/a 1a 4)) (move 1d 0a))", needToTrain=True)
    T("right angle", "((move (*d 1d 2) (/a 1a 4)) (move 1d 0a))", needToTrain=True)
    T(
        "right angle epsilon",
        "((move epsilonLength (/a 1a 4)) (move epsilonLength 0a))",
        needToTrain=True,
    )

    T("line segment", "(move 1d 0a)", needToTrain=True)

    T(
        "square slanted by 2pi/3",
        """((move 0d (/a 1a 3))
      (loop k 4 (move 1d (/a 1a 4))))""",
        needToTrain=True,
    )
    T(
        "semicircle slanted by 2pi/5",
        """((move 0d (/a 1a 5))
      (loop i infinity
    ...
    T(
        "smooth spiral 4 slanted by 2pi/2",
        """
          ((move 0d (/a 1a 2))
      (loop i infinity 
    ...
    for n, l in [(4, 2), (5, 3), (6, 4), (3, 1)]:
        T(
            "%d-circle flower l=%d" % (n, l),
            """
          (loop j %d
          (move 0d (/a 1a %d))
          (embed (loop i infinity
    ...
    for n, l in [(3, 1), (2, 2), (1, 3), (2, 1), (1, 2), (1, 1)]:
        T(
            "%d-semicircle sequence L=%d" % (n, l),
            """
          (loop j %d
          (loop i infinity
          (move (*d epsilonLength %d) epsilonAngle))
    ...
    T(
        "line next to semicircle",
        """
      ((move 1d 0a) (p (move 1d 0a)) (loop i infinity (move epsilonLength epsilonAngle)))
      """,
        needToTrain=True,
    )
    for n, l in [(3, "(/d 1d 2)"), (4, "(/d 1d 3)")]:
        T(
            "%d dashed lines of size %s" % (n, l),
    ...
    with random_seed(42):  # carefully selected for maximum entropy
        for n in [3, 4, 5, 6, 7]:
            ...
            for name in body:
                if name == "spiral" and n not in [3, 5]:
                    continue
                if name == "square" and n not in [5, 3, 6, 7]:
                    continue
                if name == "semicircle" and n not in [5, 3, 4, 6]:
                    continue
                if name == "Greek" and n not in [3, 5]:
                    continue
                if name == "double dashed" and n not in [6, 4, 3]:
                    continue
...
def montageTasks(tasks, prefix="", columns=None, testTrain=False):
    ...
    if testTrain:
        arrays = [a for a, t in zip(arrays, tasks) if t.mustTrain] + [
            a for a, t in zip(arrays, tasks) if not t.mustTrain
        ]

    arrays = [np.array([a[i : i + w] for i in range(0, len(a), w)]) for a in arrays]
    ...
    scipy.misc.imsave(
        "/tmp/%srandomMontage.png" % prefix, montage(arrays, columns=columns)
    )
...
def demoLogoTasks():
    ...
    drawLogo(
        *programs,
        ...
        smoothPretty=False,
        resolution=512,
        filenames=[f"/tmp/dreams_0/{n}_pretty.png" for n in range(len(programs))],
        timeout=1,
    )
    ...
    for n, t in enumerate(tasks):
        ...
        w = int(len(a) ** 0.5)
        scipy.misc.imsave(
            "/tmp/logo%d.png" % n, np.array([a[i : i + w] for i in range(0, len(a), w)])
        )
        logo_safe_name = (
            t.name.replace("=", "_")
            .replace(" ", "_")
