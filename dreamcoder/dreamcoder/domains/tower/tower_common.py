...
def renderPlan(
    ...
    if drawHand is not None and drawHand is not False:
        plan, drawHand = centerTower(plan, drawHand, masterPlan=masterPlan)
    else:
        plan = centerTower(plan, masterPlan=masterPlan)

    world = simulateWithoutPhysics(plan, ordered=randomSeed is None)
    ...
    def bump(x, y, c):
        ...
        x1 = x - size / 2
        x2 = x + size / 2
        a[clip(y2) : clip(y1), clip(x1) : clip(x2), :] = c
    ...
    def rectangle(x1, x2, y1, y2, c, cp=None):
        ...
        if cp is not None:
            a[
                clip(y2 + borderSize) : clip(y1 - borderSize),
    ...
    for x, y, w, h in world:
        ...
        if pretty:
            thisColor = color()
            rectangle(x1, x2, y1, y2, thisColor)
            if Lego:
                bumps = w
            ...
            rectangle(x1, x2, y1, y2, borderColor, bodyColor)

    a[resolution - floorHeight :, :, :] = 1.0
    if drawHand is not None:
        if not Lego:
            dh = 0.25
            rectangle(drawHand - dh, drawHand + dh, -99999, 99999, (0, 1, 0))
        else:
            rectangle(drawHand - 1, drawHand + 1, 43, 45, (1, 1, 1))
