...
class SupervisedTower(Task):
    def __init__(self, name, program, mustTrain=False):
        ...
        super(SupervisedTower, self).__init__(
            name, arrow(ttower, ttower), [], features=[]
        )
    ...
    def getImage(self, drawHand=False, pretty=False):
        if not drawHand:
            ...
            self.handImage = renderPlan(self.plan, drawHand=self.hand, pretty=pretty)
            return self.handImage
    ...
    def showMany(ts):
        ...
        a = montage(
            [
                renderPlan(
                    t.plan, pretty=True, Lego=True, resolution=256, drawHand=False
                )
                for t in ts
            ]
        )
    ...
    def exportImage(self, f, pretty=True, Lego=True, drawHand=False):
        a = renderPlan(
            self.plan, pretty=pretty, Lego=Lego, drawHand=t.hand if drawHand else None
        )
        import scipy.misc
...
def parseTower(s):
    ...
    def command(k, environment, continuation):
        ...
        if k[0] == Symbol("r"):
            return Application(
                Application(_r, expression(k[1], environment)), continuation
            )
        if k[0] == Symbol("l"):
            return Application(
                Application(_l, expression(k[1], environment)), continuation
            )
        if k[0] == Symbol("for"):
            v = k[1]
            ...
            return Application(
                Application(Application(_lp, b), Abstraction(Abstraction(body))),
                continuation,
            )
        if k[0] == Symbol("embed"):
    ...
    def expression(e, environment):
        ...
        if e[0] == Symbol("+"):
            return Application(
                Application(_addition, expression(e[1], environment)),
                expression(e[2], environment),
            )
        if e[0] == Symbol("-"):
            return Application(
                Application(_subtraction, expression(e[1], environment)),
                expression(e[2], environment),
            )
        assert False
...
def makeSupervisedTasks():
    arches = [
        SupervisedTower(
            "arch leg %d" % n, "((for i %d v) (r 4) (for i %d v) (l 2) h)" % (n, n)
        )
        for n in range(1, 9)
    ]
    archesStacks = [
...
"""
    ...
    simpleLoops = (
        [
            SupervisedTower(
                "horizontal row %d, spacing %d" % (n, s),
                """(for j %d h (r %s))""" % (n, s),
            )
            for n, s in [(4, 6), (5, 7)]
        ]
        + [
            SupervisedTower("horizontal stack %d" % n, """(for j %d h)""" % n)
            for n in range(5, 8)
        ]
        + [
            SupervisedTower("vertical stack %d" % n, """(for j %d v)""" % n)
            for n in [5, 7]
        ]
    )
    pyramids = []
...
def dSLDemo():
    ...
    DSL["bricks"] = [
        [
            bricks.runWithArguments([x, y + 4, _empty_tower, TowerState()])[1]
    ...
    DSL["bridge"] = [
        [
            bridge.runWithArguments([x, y, _empty_tower, TowerState()])[1]
    ...
    DSL["staircase"] = [
        staircase.runWithArguments([n, _empty_tower, TowerState()])[1]
        for n in range(4, 5 + 3)
    ]

    pyramid = Program.parse(
        "(lambda (tower_loopM $0 (lambda (lambda (moveHand 6 (tower_embed (lambda (reverseHand ((lambda (lambda (tower_loopM $1 (lambda (lambda (moveHand $2 (1x3 (moveHand 2 (tower_embed (lambda (moveHand 2 (1x3 $0))) (3x1 $0)))))))))) $2 1 $0))) $0))))))"
    )
    DSL["pyramid"] = [
        pyramid.runWithArguments([n, _empty_tower, TowerState()])[1]
        for n in range(4, 5 + 3)
    ]

    towerArch = Program.parse(
        "(lambda (lambda ((lambda ((lambda (lambda (lambda (tower_loopM $0 (lambda (lambda (1x3 (moveHand 4 ($3 $0))))) (moveHand 2 (3x1 $2)))))) $0 (lambda (reverseHand (1x3 $0))))) $0 $1)))"
    )
    DSL["towerArch"] = [
        towerArch.runWithArguments([n, _empty_tower, TowerState()])[1]
        for n in range(4, 5 + 3)
    ]
    ...
    for k, v in DSL.items():
        ...
        if d == 1:
            ...
            i = montageMatrix(
                [[renderPlan(p, pretty=True, Lego=True) for p in ps] for ps in v]
            )
        else:
            assert False
...
if __name__ == "__main__":
    ...
    print(
        ...
        max(towerHeight(simulateWithoutPhysics(f.plan)) for f in ts),
    )
    ...
    for j, t in enumerate(ts):
        t.exportImage("/tmp/tower_%d.png" % j, drawHand=False)
    ...
    SupervisedTower.exportMany(
        "/tmp/tower_montage.png",
    ...
    for n in range(100):
        ...
        SupervisedTower.exportMany(
            "/tmp/tower10_%d.png" % n, examples, columns=int(len(examples) / 2)
        )
