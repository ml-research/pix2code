...
common_primitives = (
    [
        Primitive(
            "tower_loopM",
            arrow(tint, arrow(tint, ttower, ttower), ttower, ttower),
            _simpleLoop,
        ),
...
def executeTower(p, timeout=None):
    try:
        return runWithTimeout(
            lambda: p.evaluate([])(_empty_tower)(TowerState())[1], timeout=timeout
        )
...
def animateTower(exportPrefix, p):
    ...
    assert tuple(z for z in trajectory if not isinstance(z, TowerState)) == tuple(
        actions
    )
    ...
    animation = [
        renderPlan(
            [b for b in trajectory[:n] if not isinstance(b, TowerState)],
            pretty=True,
            Lego=True,
            drawHand=hd(n),
            masterPlan=actions,
