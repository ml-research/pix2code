...
from dreamcoder.domains.tower.tower_common import (centerTower, renderPlan,
                                                   towerLength)
...
try:  # pypy will fail
    ...
    class TowerCNN(nn.Module):
        ...
        def featuresOfTask(self, t, t2=None):  # Take a task and returns [features]
            return self(
                t.getImage(), None if t2 is None else t2.getImage(drawHand=True)
            )
        ...
        def featuresOfTasks(self, ts, t2=None):  # Take a task and returns [features]
            ...
            if t2 is None:
                ...
                t2 = np.array(
                    [
                        t.getImage(drawHand=True)
                        if t
                        else np.zeros(
                            (self.inputImageDimension, self.inputImageDimension, 3)
                        )
                        for t in t2
                    ]
                )
            ...
            return self(np.array([t.getImage() for t in ts]), t2)

        def taskOfProgram(self, p, t, lenient=False):
            try:
                pl = executeTower(p, 0.05)
                if pl is None or (not lenient and len(pl) == 0):
                    return None
...
def tower_options(parser):
    parser.add_argument("--tasks", choices=["old", "new"], default="old")
    parser.add_argument("--visualize", default=None, type=str)
    parser.add_argument("--solutions", default=None, type=str)
    parser.add_argument("--split", default=1.0, type=float)
    parser.add_argument("--dream", default=None, type=str)
    parser.add_argument("--primitives", default="old", type=str, choices=["new", "old"])
...
def dreamOfTowers(grammar, prefix, N=250, make_montage=True):
    ...
    randomTowers = [
        tuple(centerTower(t))
        for _ in range(N)
        for program in [grammar.sample(request, maximumDepth=12, maxAttempts=100)]
        if program is not None
        for t in [executeTower(program, timeout=0.5) or []]
        if len(t) >= 1 and len(t) < 100 and towerLength(t) <= 360.0
    ]
...
def visualizePrimitives(primitives, fn=None):
    ...
    from dreamcoder.domains.tower.towerPrimitives import (TowerState, _embed,
                                                          _empty_tower, _left,
                                                          _loop, _right)
    ...
    _tallArch = lambda h, z, k: _lp(h, lambda _: _13(r(2, _13(l(2, z)))), r(1, _31(k)))
...
def main(arguments):
    ...
    g0 = Grammar.uniform(
        {"new": new_primitives, "old": primitives}[arguments.pop("primitives")],
    ...
    for result in generator:
        ...
        try:
            ...
            dreamOfTowers(
                result.grammars[-1], "%s/random_%d" % (outputDirectory, iteration)
            )
        except ImportError:
