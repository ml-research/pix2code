        ...
        if routine.is_parametric():
            ...
            for params in map(
                ...
                try:
                    ...
                    examples = [((inp,), routine.eval(inp, **params)) for inp in inps]
...
def make_list_bootstrap_tasks():
    ...
    def randomList(minimum=0, minimumLength=4, maximumLength=6):
        return [
            randint(minimum, 9) for _ in range(randint(minimumLength, maximumLength))
        ]
    ...
    lengthBootstrap = [
        ...
        Task(
            ...
            [((l,), len(l)) for _ in range(10) for l in [randomList()]],
        ),
        Task(
            "map length",
            arrow(tlist(tlist(tint)), tlist(tint)),
            [
    ...
    unfoldBootstrap = [
        Task(
            ...
            [((n,), list(range(n + 1, 1, -1))) for n in range(10)],
        ),
        Task(
            "weird count",
            arrow(tint, tlist(tint)),
            [((n,), list(range(-n, 0, -1))) for n in range(-10, 0)],
        ),
        Task(
            "take every other",
        ...
        Task(
            "range",
            arrow(tint, tlist(tint)),
            [((n,), list(range(n))) for n in range(10)],
        ),
        Task(
            "range inclusive",
            arrow(tint, tlist(tint)),
            [((n,), list(range(n + 1))) for n in range(10)],
        ),
    ...
    sliceBootstrap = [
        Task(
            "take bool",
            arrow(tint, tlist(tbool), tlist(tbool)),
            [
                ((n, l), l[:n])
                for n in range(10)
    ...
    foldBootstrap = [
        ...
        Task(
            "sum",
            arrow(tlist(tint), tint),
            [((l,), sum(l)) for _ in range(10) for l in [randomList()]],
        ),
        ...
        Task(
            "append constant 0",
            arrow(tlist(tint), tlist(tint)),
            [((l,), l + [0]) for _ in range(10) for l in [randomList()]],
        ),
    ]
    ...
    zipBootstrap = [
        Task(
            "zip plus",
            arrow(tlist(tint), tlist(tint), tlist(tint)),
            [
...
def bonusListProblems():
    ...
    bonus = [
        Task(
            ...
            arrow(tlist(tint), tlist(tint)),
            [
                ((x,), [x[j + (1 if j % 2 == 0 else -1)] for j in range(len(x))])
                for _ in range(5)
        ...
        Task(
            "reverse duplicate each element",
            arrow(tlist(tint), tlist(tint)),
...
def sortBootstrap():
    ...
    minimumBootstrap = [
        Task(
            ...
            [((x, y), min(x, y)) for x in range(4) for y in range(4)],
        ),
        Task(
            "minimum of list",
            arrow(tlist(tint), tint),
            [((l,), min(l)) for _ in range(15) for l in [randomList()]],
        ),
    ]
    ...
    appendBootstrap = [
        Task(
            "append bool",
            arrow(tlist(tbool), tlist(tbool), tlist(tbool)),
            [
                ((x, y), x + y)
                for _ in range(10)
                for [x, y] in [[randomBooleanList(), randomBooleanList()]]
    ...
    slowSort = [
        Task(
            "+1 maximum list",
            arrow(tlist(tint), tint),
            [((l,), max(l) + 1) for _ in range(15) for l in [randomList()]],
        ),
        Task(
...
if __name__ == "__main__":
    ...
    for t in (
        retrieveJSONTasks("data/list_tasks.json")
        + sortBootstrap()
