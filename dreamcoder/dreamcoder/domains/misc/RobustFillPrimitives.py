...
from dreamcoder.type import arrow, baseType, tint  # , t0, t1, t2
...
def _getspan(r1):
    return lambda i1: lambda b1: lambda r2: lambda i2: lambda b2: lambda string: string[
        [m.end() for m in re.finditer(r1, string)][i1]
        if b1 == "End"
        else [m.start() for m in re.finditer(r1, string)][i1] : [
            m.end() for m in re.finditer(r2, string)
        ][i2]
        if b2 == "End"
        else [m.start() for m in re.finditer(r2, string)][i2]
    ]
    # TODO format correctly


def _getspan_const(r1):
    return lambda i1: lambda b1: lambda r2: lambda i2: lambda b2: (
        defaultdict(
            int,
            {r1: i1 + 1 if i1 >= 0 else abs(i1), r2: i2 + 1 if i2 >= 0 else abs(i2)},
        ),
        max(i1 + 1 if i1 >= 0 else abs(i1), i2 + 1 if i2 >= 0 else abs(i2)),
    )
...
def _replace(d1, d2):
    return lambda string: string.replace(d1, d2)


def _getall(tp):
    return lambda string: "".join(re.findall(tp, string))


def _getfirst(tp, i):
    return lambda string: "".join(re.findall(tp, string)[:i])


def _gettoken(tp, i):
    return lambda string: re.findall(tp, string)[i]


def _gettoken_const(tp, i):
    return defaultdict(
        int, {tp: i + 1 if i >= 0 else abs(i)}
    ), i + 1 if i >= 0 else abs(i)


def _getupto(reg):
    return lambda string: string[: [m.end() for m in re.finditer(reg, string)][0]]


def _getfrom(reg):
    return lambda string: string[[m.end() for m in re.finditer(reg, string)][-1] :]


def _concat2(expr1):
    return lambda expr2: lambda string: expr1(string) + expr2(
        string
    )  # More concats plz


def _concat1(expr):
    return lambda string: expr(string)


def _concat_list(expr):
    return lambda program: lambda string: expr(string) + program(string)
...
def robustFillPrimitives(max_len=100, max_index=5):
    return (
        [
            ...
            CPrimitive(
                "concat_list", arrow(texpression, tprogram, tprogram), _concat_list
            ),
            # expressions
            CPrimitive(
                "Constant", arrow(tcharacter, texpression), lambda x: lambda y: x
            ),  # add a constraint
            CPrimitive(
                "apply",
                arrow(tnesting, tsubstr, texpression),
                lambda n: lambda sub: lambda string: n(sub(string)),
            ),
            CPrimitive(
                "apply_n",
                arrow(tnesting, tnesting, texpression),
        ...
        + [
            # substrings
            CPrimitive(
                "SubStr", arrow(tposition, tposition, tsubstr), _substr
            ),  # handled
            CPrimitive(
                "GetSpan",
                arrow(tregex, tindex, tboundary, tregex, tindex, tboundary, tsubstr),
                _getspan,
                _getspan_const,
            ),  # TODO constraint
        ]
        ...
        + [
            CPrimitive(
                "Replace_" + name1 + name2,
                tnesting,
                _replace(char1, char2),
                (defaultdict(int, {char1: 1}), 1),
            )
            for name1, char1 in delim_dict.items()
            for name2, char2 in delim_dict.items()
        ...
        + [
            # regexes
            CPrimitive(
                "type_to_regex", arrow(ttype, tregex), lambda x: x
            ),  # TODO also make disappear
            CPrimitive(
                "delimiter_to_regex", arrow(tdelimiter, tregex), lambda x: re.escape(x)
            ),  # TODO also make disappear
        ]
        + [
            # types
...
def RobustFillProductions(max_len=100, max_index=5):
    return [
        (0.0, prim)
        for prim in robustFillPrimitives(max_len=max_len, max_index=max_index)
    ]
        ...
        if constraint is None:
            if len(ty.functionArguments()) == 0:
                ...
                for _ in range(len(ty.functionArguments()) - 1):
                    self.constraint = lambda x: lambda y: add_constraints(
                        x, self.constraint(y)
                    )
        else:
