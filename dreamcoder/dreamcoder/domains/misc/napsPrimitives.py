...
from dreamcoder.type import arrow, baseType, tint, tlist  # , t0, t1, t2
...
def _program(records):
    return lambda funcs: {"types": records, "funcs": funcs}


# record
def _func(string):
    return lambda tp: lambda name: lambda vars1: lambda vars2: lambda stmts: [
        string,
        tp,
        name,
        vars1,
...
def _var(tp):
    return lambda name: ["var", tp, name]


# stmt
# expr
def _assign(tp):
    return lambda lhs: lambda expr: ["assign", tp, lhs, expr]


# lhs
def _if(tp):
    return lambda expr: lambda stmts1: lambda stmts2: [
        "if",
        tp,
        expr,
        stmts1,
...
def _foreach(tp):
    return lambda var: lambda expr: lambda stmts: ["foreach", tp, expr, stmts]  # TODO


def _while(tp):
    return lambda expr: lambda stmts1: lambda stmts1: [
        "while",
        tp,
        expr,
        stmts1,
        stmts2,
    ]  # or: ['while', tp, expr, [stmts1], [stmts2]] #TODO


# break
# continue
def _return(tp):
    return lambda expr: ["return", tp, expr]


# noop
def _field(tp):
    return lambda expr: lambda field_name: ["field", tp, expr, field_name]


def _constant(tp):
    return lambda value: ["val", tp, value]  # TODO deal with value


def _invoke(tp):
    return lambda function_name: lambda exprs: [
        "invoke",
        tp,
        function_name,
        exprs,
    ]  # TODO, deal with fn_name and lists


def _ternary(tp):
    return lambda expr1: lambda expr2: lambda expr3: ["?:", tp, expr1, expr2, expr3]


def _cast(tp):
    return lambda expr: ["cast", tp, expr]
...
def napsPrimitives():
    return (
        [
            Primitive(
                "program", arrow(tlist(RECORD), tlist(FUNC), PROGRAM), _program
            ),  # TODO
            # RECORD
            Primitive(
                "func",
        ...
        + [
            ...
            Primitive(
                "foreach", arrow(TYPE, VAR, EXPR, tlist(STMT), FOREACH), _foreach
            ),
            Primitive(
                "while", arrow(TYPE, EXPR, tlist(STMT), tlist(STMT), WHILE), _while
            ),
            ...
            Primitive(
                "invoke", arrow(TYPE, function_name, tlist(EXPR), INVOKE), _invoke
            ),  # TODO
        ...
        + [
            #
            Primitive(f"var{str(i)}", name, f"var{str(i)}")
            for i in range(12)
        ]
    )
...
if __name__ == "__main__":
    ...
    g = Grammar.fromProductions(deepcoderProductions(), logVariable=0.9)
