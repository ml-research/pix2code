...
from dreamcoder.domains.regex.makeRegexTasks import (makeHandPickedTasks,
                                                     makeLongTasks,
                                                     makeNewNumberTasks,
                                                     makeNewTasks,
                                                     makeNumberTasks,
                                                     ...
                                                     makeShortTasks,
                                                     makeWordTasks)
from dreamcoder.domains.regex.regexPrimitives import (PRC, alt2Primitives,
                                                      altPrimitives,
                                                      basePrimitives,
                                                      concatPrimitives,
                                                      easyWordsPrimitives,
                                                      reducedConcatPrimitives,
                                                      strConstConcatPrimitives)
from dreamcoder.dreamcoder import Task, explorationCompression
...
from dreamcoder.utilities import (POSITIVEINFINITY, eprint, flatten,
                                  testTrainSplit)

try:
    from dreamcoder.recognition import (JSONFeatureExtractor,
    ...
    class LearnedFeatureExtractor(RecurrentFeatureExtractor):
        ...
        def taskOfProgram(self, p, t):
            ...
            p = p.visit(ConstantInstantiateVisitor())

            preg = p.evaluate([])(pre.String(""))
            t = Task(
                "Helm", t, [((), list(preg.sample())) for _ in range(num_examples)]
            )
            return t
...
def regex_options(parser):
    ...
    parser.add_argument(
        "-H", "--hidden", type=int, default=256, help="number of hidden units"
    )
    parser.add_argument(
        "--likelihoodModel",
    ...
    parser.add_argument(
        "--topk_use_map", dest="topk_use_only_likelihood", action="store_false"
    )
    ...
    parser.add_argument(
        "--use_str_const", action="store_true", help="use string constants"
    )
...
def main(args):
    ...
    if not use_ll_cutoff is False:
        ...
        if len(use_ll_cutoff) == 1:
            train_ll_cutoff = use_ll_cutoff[
                0
            ]  # make_cutoff_model(use_ll_cutoff[0], tasks))
            test_ll_cutoff = use_ll_cutoff[
                0
            ]  # make_cutoff_model(use_ll_cutoff[0], tasks))
        else:
            ...
            test_ll_cutoff = use_ll_cutoff[
                1
            ]  # make_cutoff_model(use_ll_cutoff[1], tasks))
    else:
    ...
    eprint(
        "added cutoff values to tasks, train: ",
        train_ll_cutoff,
        ", test:",
        test_ll_cutoff,
    )
    ...
    for task in test + train:
        ...
        task.specialTask = (
            "regex",
            {"cutoff": task.ll_cutoff, "str_const": task.str_const},
        )
        task.examples = [(xs, [y for y in ys]) for xs, ys in task.examples]
    ...
    extractor = {"learned": LearnedFeatureExtractor, "json": MyJSONFeatureExtractor}[
        args.pop("extractor")
    ]
    ...
    productions = [
        (math.log(0.5 / float(n_base_prim)), prim)
        if prim.name not in specials
        else (math.log(0.10), prim)
    ...
    if test_stuff:
        ...
        eprint(
            """half the probability mass is on higher-order primitives.
