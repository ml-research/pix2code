...
try:
    ...
    print(
        "Failure to load pregex. This is only acceptable if using pypy", file=sys.stderr
    )
    ...
    def __call__(self, pre):
        ...
        if self.arity == len(self.args):
            if self.arity == 0:
                ...
                return pregex.Concat(
                    [self.f(self.args), pre]
                )  # this line is bad, need brackets around input to f if f is Alt
        else:
...
def concatPrimitives():
    return (
        ...
        + [
            Primitive(
                "string_" + name, arrow(tpregex, tpregex), PRC(pregex.String(char))
            )
...
def emp_dot(corpus):
    return pregex.CharacterClass(
        printable[:-4], emp_distro_from_corpus(corpus, printable[:-4]), name="."
    )


def emp_d(corpus):
    return pregex.CharacterClass(
        printable[:10], emp_distro_from_corpus(corpus, printable[:10]), name="\\d"
    )
...
def emp_dot_no_letter(corpus):
    return pregex.CharacterClass(
        printable[:10] + printable[62:],
        emp_distro_from_corpus(corpus, printable[:10] + printable[62:]),
        name=".",
    )
...
def emp_w(corpus):
    return pregex.CharacterClass(
        printable[:62], emp_distro_from_corpus(corpus, printable[:62]), name="\\w"
    )


def emp_l(corpus):
    return pregex.CharacterClass(
        printable[10:36], emp_distro_from_corpus(corpus, printable[10:36]), name="\\l"
    )


def emp_u(corpus):
    return pregex.CharacterClass(
        printable[36:62], emp_distro_from_corpus(corpus, printable[36:62]), name="\\u"
    )
