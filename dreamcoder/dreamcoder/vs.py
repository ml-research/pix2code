...
class VersionTable:
    def __init__(self, typed=True, identity=True, factored=False):
        ...
        if self.debug:
            print(
                "WARNING: running version spaces in debug mode. Will be substantially slower."
            )
    ...
    def branchingFactor(self, j):
        ...
        if l.isApplication:
            return max(self.branchingFactor(l.f), self.branchingFactor(l.x))
        if l.isUnion:
            return max([len(l.elements)] + [self.branchingFactor(e) for e in l])
        if l.isAbstraction:
            return self.branchingFactor(l.body)
        return 0
    ...
    def intention(self, j, isFunction=False):
        ...
        if l.isIndex or l.isPrimitive or l.isInvented:
            return l
        if l.isAbstraction:
            return Abstraction(self.intention(l.body))
        if l.isApplication:
            return Application(self.intention(l.f), self.intention(l.x))
        if l.isUnion:
            return Union(self.intention(e) for e in l)
        assert False
    ...
    def incorporate(self, p):
        ...
        if p.isIndex or p.isPrimitive or p.isInvented:
            ...
            p = Application(self.incorporate(p.f), self.incorporate(p.x))
        elif p.isUnion:
            if len(p.elements) > 0:
    ...
    def _incorporate(self, p):
        if p in self.expression2index:
            return self.expression2index[p]
    ...
    def intersection(self, a, b):
        ...
        if x.isApplication and y.isApplication:
            return self.apply(self.intersection(x.f, y.f), self.intersection(x.x, y.x))
        if x.isUnion:
            if y.isUnion:
                return self.union([self.intersection(x_, y_) for x_ in x for y_ in y])
            return self.union([self.intersection(x_, b) for x_ in x])
        if y.isUnion:
            return self.union([self.intersection(a, y_) for y_ in y])
        return self.empty
    ...
    def haveOverlap(self, a, b):
        ...
        if x.isAbstraction and y.isAbstraction:
            ...
            overlap = self.haveOverlap(x.f, y.f) and self.haveOverlap(x.x, y.x)
        elif x.isUnion:
            if y.isUnion:
                overlap = any(self.haveOverlap(x_, y_) for x_ in x for y_ in y)
            overlap = any(self.haveOverlap(x_, b) for x_ in x)
        elif y.isUnion:
            overlap = any(self.haveOverlap(a, y_) for y_ in y)
        else:
            overlap = False
    ...
    def minimalInhabitants(self, j):
        ...
        if self.inhabitantTable[j] is not None:
            return self.inhabitantTable[j]
        ...
        if e.isAbstraction:
            ...
            cost = fc + xc + epsilon
            members = {self.apply(f_, x_) for f_ in fm for x_ in xm}
        elif e.isUnion:
            children = [self.minimalInhabitants(z) for z in e]
            cost = min(c for c, _ in children)
            members = {zp for c, z in children if c == cost for zp in z}
        else:
    ...
    def minimalFunctionInhabitants(self, j):
        """Returns (minimal size, set of singleton version spaces)"""
        assert isinstance(j, int)
        if self.functionInhabitantTable[j] is not None:
            return self.functionInhabitantTable[j]
        e = self.expressions[j]
        if e.isAbstraction:
            ...
            children = [self.minimalFunctionInhabitants(z) for z in e]
            cost = min(c for c, _ in children)
            members = {zp for c, z in children if c == cost for zp in z}
        else:
            assert e.isIndex or e.isInvented or e.isPrimitive
    ...
    def shiftFree(self, j, n, c=0):
        ...
        l = self.expressions[j]
        if l.isUnion:
            return self.union([self.shiftFree(e, n, c) for e in l])
        if l.isApplication:
            return self.apply(self.shiftFree(l.f, n, c), self.shiftFree(l.x, n, c))
        if l.isAbstraction:
    ...
    def _substitutions(self, j, n):
        if (j, n) in self.substitutionTable:
            return self.substitutionTable[(j, n)]

        s = self.shiftFree(j, n)
        if self.debug:
            assert set(self.extract(s)) == set(
                e.shift(-n)
        ...
        if l.isPrimitive or l.isInvented:
            ...
            for v1, f in fm.items():
                ...
                for v2, x in xm.items():
                    ...
                    if self.typed:
                        if self.infer(a) == self.bottom:
                            continue
                        try:
                            nType = canonicalUnification(
                                nType1, nType2, self.infer(a)[0].get(n, t0)
                            )
                    ...
                    if v == self.empty:
                        continue
                    if self.typed and self.infer(v) == self.bottom:
                        continue
            ...
            for v1, f in fm.items():
                ...
                for v2, x in xm.items():
                    ...
                    if v == self.empty:
                        continue
                    if v in newMapping:
                        newMapping[v] = ({f} | newMapping[v][0], {x} | newMapping[v][1])
                    else:
                        newMapping[v] = ({f}, {x})
    ...
    def recursiveInversion(self, j):
        if self.recursiveTable[j] is not None:
            return self.recursiveTable[j]
    ...
    def rewriteReachable(self, heads, n):
        ...
        spaces = {v: self.repeatedExpansion(v, n) for v in vertices}
        return spaces
    ...
    def loadEquivalences(self, g, spaces):
        ...
        def extract(j):
            ...
            l = self.expressions[j]
            if l.isAbstraction:
                ks = g.setOfClasses(g.abstractClass(b) for b in extract(l.body))
            elif l.isApplication:
                fs = extract(l.f)
                xs = extract(l.x)
                ks = g.setOfClasses(g.applyClass(f, x) for x in xs for f in fs)
            elif l.isUnion:
    ...
    def bestInventions(self, versions, bs=25):
        ...
        def nontrivial(proposal):
            ...
            for d, tree in proposal.walk():
                if tree.isPrimitive or tree.isInvented:
                    primitives += 1
                elif tree.isIndex:
                    i = tree.i - d
                    if i in indices:
        ...
        with timing("calculated candidates from version space"):
            candidates = [
                {
                    ...
                    for _, js in [
                        self.minimalInhabitants(k),
                        self.minimalFunctionInhabitants(k),
                    ]
                    for j in js
                }
                for hs in versions
            ...
            candidates = {
                ...
                if f >= 2 and nontrivial(next(self.extract(k)))
            }
            ...
            candidateCost = {
                k: len(set(next(self.extract(k)).freeVariables())) + 1
                for k in candidates
            }
        ...
        class B:
            def __init__(self, j):
                ...
                self.relativeCost = {
                    inhabitant: candidateCost[inhabitant]
                    for inhabitant in inhabitants
                    if inhabitant in candidates
                }
                self.relativeFunctionCost = {
                    inhabitant: candidateCost[inhabitant]
            ...
            def restrict(self):
                if len(self.relativeCost) > bs:
                    self.relativeCost = dict(
                        sorted(self.relativeCost.items(), key=lambda rk: rk[1])[:bs]
                    )
                if len(self.relativeFunctionCost) > bs:
                    self.relativeFunctionCost = dict(
                        sorted(self.relativeFunctionCost.items(), key=lambda rk: rk[1])[
                            :bs
                        ]
                    )
            ...
            def relax(self, given, cost):
                self.relativeCost[given] = min(cost, self.getCost(given))

            def relaxFunction(self, given, cost):
                self.relativeFunctionCost[given] = min(
                    cost, self.getFunctionCost(given)
                )

            def unobject(self):
                return {
                    "relativeCost": self.relativeCost,
                    "defaultCost": self.defaultCost,
                    "relativeFunctionCost": self.relativeFunctionCost,
                    "defaultFunctionCost": self.defaultFunctionCost,
                }
        ...
        def costs(j):
            ...
            if e.isIndex or e.isPrimitive or e.isInvented:
                ...
                for i in f.functionDomain | x.domain:
                    ...
                    beamTable[j].relaxFunction(
                        i, f.getFunctionCost(i) + x.getCost(i) + epsilon
                    )
            elif e.isUnion:
                for z in e:
                    cz = costs(z)
                    for i, c in cz.relativeCost.items():
                        beamTable[j].relax(i, c)
                    for i, c in cz.relativeFunctionCost.items():
                        beamTable[j].relaxFunction(i, c)
            else:
                assert False
        ...
        candidates = {d for _bs in beams for b in _bs for d in b["relativeCost"].keys()}

        def score(candidate):
            return sum(
                min(
                    min(
                        b["relativeCost"].get(candidate, b["defaultCost"]),
                        b["relativeFunctionCost"].get(
                            candidate, b["defaultFunctionCost"]
                        ),
                    )
                    for b in _bs
                )
    ...
    def rewriteWithInvention(self, i, js):
        ...
        def rewrite(j):
            if j in table:
                return table[j]
            e = self.expressions[j]
            if self.haveOverlap(i, j):
                r = RW(fc=1, ac=1, f=_i, a=_i)
            elif e.isPrimitive or e.isInvented or e.isIndex:
                r = RW(fc=1, ac=1, f=e, a=e)
            elif e.isApplication:
                f = rewrite(e.f)
                x = rewrite(e.x)
                cost = f.fc + x.ac + epsilon
                ep = Application(f.f, x.a) if cost < POSITIVEINFINITY else None
                r = RW(fc=cost, ac=cost, f=ep, a=ep)
            elif e.isAbstraction:
                b = rewrite(e.body)
                cost = b.ac + epsilon
                ep = Abstraction(b.a) if cost < POSITIVEINFINITY else None
                r = RW(f=None, fc=POSITIVEINFINITY, a=ep, ac=cost)
            elif e.isUnion:
                children = [rewrite(z) for z in e]
                f, fc = min(((child.f, child.fc) for child in children), key=cindex(1))
                a, ac = min(((child.a, child.ac) for child in children), key=cindex(1))
                r = RW(f=f, fc=fc, a=a, ac=ac)
            else:
                assert False
            table[j] = r
            return r
    ...
    def addInventionToGrammar(self, candidate, g0, frontiers, pseudoCounts=1.0):
        candidateSource = next(self.extract(candidate))
        v = RewriteWithInventionVisitor(candidateSource)
        invention = v.invention

        rewriteMapping = list({e.program for f in frontiers for e in f})
        spaces = [
            self.superCache[self.incorporate(program)] for program in rewriteMapping
        ]
        rewriteMapping = dict(
            zip(rewriteMapping, self.rewriteWithInvention(candidate, spaces))
        )
        ...
        frontiers = [
            Frontier(
                [
                    FrontierEntry(
                        program=tryRewrite(e.program, request=f.task.request),
        ...
        # print()
        g = Grammar.uniform(
            [invention] + g0.primitives, continuationType=g0.continuationType
        ).insideOutside(frontiers, pseudoCounts=pseudoCounts)
...
class RewriteWithInventionVisitor:
    def __init__(self, p):
        ...
        for j in range(len(self.mapping) - 1, -1, -1):
            self.appliedInvention = Application(
                self.appliedInvention, Index(self.mapping[j])
            )
    ...
    def application(self, e):
        return self.tryRewrite(e) or Application(e.f.visit(self), e.x.visit(self))
...
def induceGrammar_Beta(
    ...
    def scoreCandidate(candidate, currentFrontiers, currentGrammar):
        try:
            newGrammar, newFrontiers = v.addInventionToGrammar(
                candidate, currentGrammar, currentFrontiers, pseudoCounts=pseudoCounts
            )
    ...
    while True:
        ...
        with timing("constructed %d-step version spaces" % arity):
            versions = [
                [v.superVersionSpace(v.incorporate(e.program), arity) for e in f]
        ...
        with timing("scored the candidate inventions"):
            scoredCandidates = parallelMap(
                CPUs,
                lambda candidate: (
                    candidate,
                    scoreCandidate(candidate, restrictedFrontiers, g0),
                ),
                candidates,
                memorySensitive=True,
                chunksize=1,
        ...
        newGrammar, newFrontiers = v.addInventionToGrammar(
            bestNew, g0, frontiers, pseudoCounts=pseudoCounts
        )
        eprint(
            "Improved score to",
            bestScore,
            ...
            bestScore - oldScore,
            ") w/ invention",
            newGrammar.primitives[0],
            ":",
            newGrammar.primitives[0].infer(),
        )
        oldScore = bestScore
...
def testTyping(p):
    ...
    j = v.incorporate(p)
    arbitrary = set(
        v.extract(v.recursiveInversion(v.recursiveInversion(v.recursiveInversion(j))))
    )
    print(len(arbitrary))
    assert wellTyped <= arbitrary
    assert wellTyped == {e for e in arbitrary if e.wellTyped()}
...
def testSharing(projection=2):
    ...
    if projection == 3:
        ...
        a.plot_surface(X, Y, np.log10(Z), color="blue", alpha=0.3)
        for l in range(L):
            for n in range(N):
                Z[l, n] = dumb[(l, n)]

        a.plot_surface(X, Y, np.log10(Z), color="red", alpha=0.3)
        ...
        for n in range(1, 2):
            ...
            if n > 1:
                dy = 1
            if n == 1 and logarithmic:
                dy = 0.6
            if n == 1 and not logarithmic:
                dy = 1
...
if __name__ == "__main__":
    ...
    with timing("induced DSL"):
        induceGrammar_Beta(
            g0,
            [Frontier.dummy(p, tp=tp) for p, tp in programs],
            CPUs=1,
            a=N,
