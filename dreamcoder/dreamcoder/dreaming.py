...
from dreamcoder.domains.arithmetic.arithmeticPrimitives import (addition, k0,
                                                                k1,
                                                                multiplication,
                                                                subtraction)
...
from dreamcoder.utilities import eprint, get_root_dir, mean, timing, tuplify
...
def helmholtzEnumeration(
    ...
    if evaluationTimeout:
        message["evaluationTimeout"] = evaluationTimeout
    if special:
...
def backgroundHelmholtzEnumeration(
    ...
    inputs = {
        r: list({tuplify(xs) for t in tasks if t.request == r for xs, y in t.examples})
        for r in requests
    }
    ...
    promises = [
        workers.apply_async(
            ...
            kwds={"special": special, "evaluationTimeout": evaluationTimeout},
        )
        for r in requests
    ]
...
if __name__ == "__main__":
    ...
    frontiers = helmholtzEnumeration(g, arrow(tint, tint), [[0], [1], [2]], 10.0)
