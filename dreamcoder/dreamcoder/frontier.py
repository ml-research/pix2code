...
class FrontierEntry(object):
    def __init__(
        self, program, _=None, logPrior=None, logLikelihood=None, logPosterior=None
    ):
        self.logPosterior = (
    ...
    def strip_primitive_values(self):
        return FrontierEntry(
            program=strip_primitive_values(self.program),
    ...
    def unstrip_primitive_values(self):
        return FrontierEntry(
            program=unstrip_primitive_values(self.program),
            logPrior=self.logPrior,
            logPosterior=self.logPosterior,
            logLikelihood=self.logLikelihood,
        )
...
class Frontier(object):
    ...
    def strip_primitive_values(self):
        return Frontier([e.strip_primitive_values() for e in self.entries], self.task)

    def unstrip_primitive_values(self):
        return Frontier([e.unstrip_primitive_values() for e in self.entries], self.task)

    DUMMYFRONTIERCOUNTER = 0

    @staticmethod
    def dummy(program, logLikelihood=0.0, logPrior=0.0, tp=None):
        """Creates a dummy frontier containing just this program"""
        if not tp:
            tp = program.infer().negateVariables()

        t = Task(
            "<dummy %d: %s>" % (Frontier.DUMMYFRONTIERCOUNTER, str(program)), tp, []
        )
        f = Frontier(
            [
                FrontierEntry(
                    program=program, logLikelihood=logLikelihood, logPrior=logPrior
                )
            ],
            task=t,
        )
    ...
    def normalize(self):
        ...
        return Frontier(newEntries, self.task)
    ...
    def expectedProductionUses(self, g):
        ...
        for j, p in enumerate(ps):
            for e in this:
                ...
                features[j] += w * sum(child == p for _, child in e.program.walk())
            if not p.isInvented:
                features[j] *= 0.3
        return features
    ...
    def topK(self, k):
        if k == 0:
            return Frontier([], self.task)
        if k < 0:
            return self
        newEntries = sorted(
            self.entries, key=lambda e: (-e.logPosterior, str(e.program))
        )
        return Frontier(newEntries[:k], self.task)

    def sample(self):
        """Samples an entry from a frontier"""
        return sampleLogDistribution([(e.logLikelihood + e.logPrior, e) for e in self])
    ...
    @property
    def bestll(self):
        best = max(self.entries, key=lambda e: e.logLikelihood)
    ...
    def summarizeFull(self):
        ...
        return "\n".join(
            [self.task.name]
            + ["%f\t%s" % (e.logPosterior, e.program) for e in self.normalize()]
        )
    ...
    def describe(frontiers):
        ...
        if numberOfHits > 0:
            averageLikelihood = (
                sum(f.bestPosterior.logPrior for f in frontiers if not f.empty)
                / numberOfHits
            )
        else:
            averageLikelihood = 0
        return "\n".join(
            [f.summarize() for f in frontiers]
            + ["Hits %d/%d tasks" % (numberOfHits, len(frontiers))]
            + [
                "Average description length of a program solving a task: %f nats"
                % (-averageLikelihood)
            ]
        )
