...
class Network(nn.Module):
    ...
    def __init__(
        ...
        if cell_type == "GRU":
            ...
            self.decoder_cell = nn.GRUCell(
                input_size=self.v_target + 1, hidden_size=self.h_decoder_size, bias=True
            )
        if cell_type == "LSTM":
        ...
        self.W = nn.Linear(
            self.h_output_encoder_size + self.h_decoder_size, self.embedding_size
        )
        self.V = nn.Linear(self.embedding_size, self.v_target + 1)
        self.input_A = nn.Bilinear(
            self.h_input_encoder_size, self.h_output_encoder_size, 1, bias=False
        )
        self.output_A = nn.Bilinear(
            self.h_output_encoder_size, self.h_decoder_size, 1, bias=False
        )
    ...
    def __setstate__(self, state):
        ...
        if isinstance(self.input_encoder_init, tuple):
            self.input_encoder_init = nn.ParameterList(list(self.input_encoder_init))
    ...
    def input_encoder_get_init(self, batch_size):
        ...
        if self.cell_type == "LSTM":
            return tuple(x.repeat(batch_size, 1) for x in self.input_encoder_init)
    ...
    def output_encoder_get_init(self, input_encoder_h):
        ...
        if self.cell_type == "LSTM":
            return (
                input_encoder_h,
                self.output_encoder_init_c.repeat(input_encoder_h.size(0), 1),
            )
    ...
    def run(self, inputs, outputs, target=None, mode="sample"):
        ...
        inputs_scatter = [
            Variable(
                torch.zeros(max_length_input[j], batch_size, self.v_input + 1).scatter_(
                    2, inputs[j][:, :, None], 1
                )
            )
        ...
        outputs_scatter = [
            Variable(
                torch.zeros(
                    max_length_output[j], batch_size, self.v_input + 1
                ).scatter_(2, outputs[j][:, :, None], 1)
            )
            for j in range(n_examples)
        ]  # n_examples * (max_length_output * batch_size * v_input+1)
        if target is not None:
            target_scatter = Variable(
                torch.zeros(max_length_target, batch_size, self.v_target + 1).scatter_(
                    2, target[:, :, None], 1
                )
        ...
        for j in range(n_examples):
            ...
            for i in range(max_length_input[j]):
                state = self.input_encoder_cell(inputs_scatter[j][i, :, :], state)
                if i + 1 < max_length_input[j]:
                    active[i + 1, :] = active[i, :] * (inputs[j][i, :] != self.v_input)
            ...
            embedding = input_H[j].gather(
                0,
                Variable(
                    embedding_idx[None, :, None].repeat(1, 1, self.h_input_encoder_size)
                ),
            )[0]
        ...
        def input_attend(j, h_out):
            ...
            scores = (
                self.input_A(
                    input_H[j].view(
                        max_length_input[j] * batch_size, self.h_input_encoder_size
                    ),
                    h_out.view(batch_size, self.h_output_encoder_size).repeat(
                        max_length_input[j], 1
                    ),
                ).view(max_length_input[j], batch_size)
                + input_attention_mask[j]
            )
        ...
        for j in range(n_examples):
            ...
            for i in range(max_length_output[j]):
                state = self.output_encoder_cell(
                    torch.cat([outputs_scatter[j][i, :, :], input_attend(j, h)], 1),
                    state,
                )
                if i + 1 < max_length_output[j]:
                    active[i + 1, :] = active[i, :] * (outputs[j][i, :] != self.v_input)
                h = self.cell_get_h(state)
                hs.append(h[None, :, :])
            ...
            embedding = output_H[j].gather(
                0,
                Variable(
                    embedding_idx[None, :, None].repeat(
                        1, 1, self.h_output_encoder_size
                    )
                ),
            )[0]
        ...
        def output_attend(j, h_dec):
            ...
            scores = (
                self.output_A(
                    output_H[j].view(
                        max_length_output[j] * batch_size, self.h_output_encoder_size
                    ),
                    h_dec.view(batch_size, self.h_decoder_size).repeat(
                        max_length_output[j], 1
                    ),
                ).view(max_length_output[j], batch_size)
                + output_attention_mask[j]
            )
        ...
        target = (
            target
            if mode == "score"
            else torch.zeros(max_length_target, batch_size).long()
        )
        decoder_states = [
            self.decoder_get_init(output_embeddings[j]) for j in range(n_examples)
        ]  # P
        ...
        for i in range(max_length_target):
            ...
            if mode == "sample":
                target[i, :] = torch.multinomial(logsoftmax.data.exp(), 1)[:, 0]
            score = score + choose(logsoftmax, target[i, :]) * Variable(active.float())
    ...
    def inputsToTensors(self, inputss):
        ...
        for j in range(len(inputss[0])):
            ...
            t = (
                torch.ones(1 if maxlen == 0 else maxlen + 1, len(inputs)).long()
                * self.v_input
            )
    ...
    def targetToTensor(self, targets):
        ...
        t = (
            torch.ones(1 if maxlen == 0 else maxlen + 1, len(targets)).long()
            * self.v_target
        )
