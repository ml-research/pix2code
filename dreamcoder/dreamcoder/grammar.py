...
class Grammar(object):
    ...
    def __hash__(self):
        return hash(self._immutable_code())
    ...
    def bestFirstEnumeration(self, request):
        ...
        def g(parentCost, request, _=None, context=None, environment=[], k=None):
            """
            k is a continuation.
                ...
                choices(
                    ...
                    [
                        (
                            ...
                            lambda: ga(
                                parentCost - f_ll_tp_newContext[1][0],
                        ...
                        for f_ll_tp_newContext in iter(candidates.items())
                    ],
                )
        ...
        def ga(
            ...
            if argumentTypes == []:
                ...
                g(
                    ...
                    context=context,
                    environment=environment,
                    k=lambda newCost, newContext, argument: ga(
                        newCost,
    ...
    def productionUses(self, frontiers):
        ...
        for f in frontiers:
            for e in f:
                summary = self.closedLikelihoodSummary(f.task.request, e.program)
    ...
    def enumeration(
        ...
        if request.isArrow():
            ...
            for l, newContext, b in self.enumeration(
                context,
                [v] + environment,
                request.arguments[1],
            ...
            for l, t, p, newContext in candidates:
                ...
                for aL, aK, application in self.enumerateApplication(
                    newContext,
                    environment,
                    p,
                    xs,
    ...
    def enumerateApplication(
        ...
        if argumentRequests == []:
            if lowerBound <= 0.0 and 0.0 < upperBound:
                yield 0.0, context, function
            else:
                return
        else:
            argRequest = argumentRequests[0].apply(context)
            laterRequests = argumentRequests[1:]
            for argL, newContext, arg in self.enumeration(
                context,
                environment,
                argRequest,
                upperBound=upperBound,
                ...
                for resultL, resultK, result in self.enumerateApplication(
                    newContext,
                    environment,
                    newFunction,
                    laterRequests,
    ...
    def sketchEnumeration(
        ...
        if sk.isHole:
            ...
            v = request.arguments[0]
            for l, newContext, b in self.sketchEnumeration(
                context,
                [v] + environment,
                request.arguments[1],
                sk.body,
            ...
            yield from self.sketchApplication(
                context,
                environment,
                f,
                xs,
    ...
    def sketchApplication(
        ...
        if argumentRequests == []:
            ...
            for argL, newContext, arg in self.sketchEnumeration(
                context,
                environment,
                argRequest,
                firstSketch,
    ...
    def sketchLogLikelihood(
        self, request, full, sk, context=Context.EMPTY, environment=[]
    ):
        """
        ...
        if sk.isHole:
            ...
            v = request.arguments[0]
            return self.sketchLogLikelihood(
                request.arguments[1],
                full.body,
                sk.body,
                context=context,
            ...
            return self.sketchllApplication(
                context, environment, sk_f, sk_xs, full_f, full_xs, argumentRequests
            )
    ...
    def sketchllApplication(
        ...
        if argumentRequests == []:
            return torch.tensor([0.0]).cuda(), context  # does this make sense?
        else:
            argRequest = argumentRequests[0].apply(context)
            laterRequests = argumentRequests[1:]
            ...
            argL, newContext = self.sketchLogLikelihood(
                argRequest,
                full_firstSketch,
                sk_firstSketch,
                context=context,
...
normalizers = {%s})""" % (
    ...
    def record(self, actual, possibles, constant=0.0):
...
class Uses(object):
    ...
    def __iadd__(self, o):
        ...
        for k, v in o.possibleUses.items():
            self.possibleUses[k] = self.possibleUses.get(k, 0.0) + v
        for k, v in o.actualUses.items():
            self.actualUses[k] = self.actualUses.get(k, 0.0) + v
        return self
...
class ContextualGrammar:
    def __init__(self, noParent, variableParent, library):
        self.noParent, self.variableParent, self.library = (
            noParent,
            variableParent,
            library,
        )
    ...
    def untorch(self):
        return ContextualGrammar(
            self.noParent.untorch(),
            self.variableParent.untorch(),
            {e: [g.untorch() for g in gs] for e, gs in self.library.items()},
        )

    def randomWeights(self, r):
        """returns a new grammar with random weights drawn from r. calls `r` w/ old weight"""
        return ContextualGrammar(
            self.noParent.randomWeights(r),
            self.variableParent.randomWeights(r),
            {e: [g.randomWeights(r) for g in gs] for e, gs in self.library.items()},
        )
    ...
    def __str__(self):
        lines = [
            ...
            "",
            "Variable parent:",
            str(self.variableParent),
            "",
            "",
        ]
        for e, gs in self.library.items():
            for j, g in enumerate(gs):
                lines.extend(["Parent %s, argument index %s" % (e, j), str(g), ""])
        return "\n".join(lines)
    ...
    def featureVector(self, _=None, requests=None, onlyInventions=True, normalize=True):
        ...
        features = []
        logWeights = []
        for l, t, p in sorted(self.noParent.productions, key=lambda z: str(z[2])):
        ...
        for parent in sorted(self.primitives, key=str):
            ...
            for j, g in enumerate(self.library[parent]):
                ...
                logWeights = []
                for l, t, p in sorted(g.productions, key=lambda z: str(z[2])):
                    if onlyInventions and not p.isInvented:
                        continue
...
def batchLikelihood(jobs):
    ...
    superGrammar = Grammar.uniform(
        list({p for _1, _2, g in jobs for p in g.primitives}),
...
class PCFG:
    ...
    def from_grammar(g, request, maximum_type=3, maximum_environment=2):
        ...
        def size_of_type(t):
            if isinstance(t, TypeVariable):
                return 0
            if t.isArrow():
                return max(size_of_type(t.arguments[0]), size_of_type(t.arguments[1]))
        ...
        def instantiations(t):
            ...
            for substitution in itertools.product(
                possible_types, repeat=len(variables)
            ):
                context = Context(
        ...
        def make_rules(request, environment):
            ...
            if g.continuationType == request:
                variable_candidates = [
                    min(variable_candidates, key=lambda vc: vc[-1].i)
                ]
            variable_candidates = [
                (lp - math.log(len(variable_candidates)), t, p)
            ...
            for lp, t, p in g.productions + variable_candidates:
                for i in instantiations(t):
                    if i.returns() == request:
                        ...
                        argument_symbols = []
                        for a in arguments:
                            new_environment = push_multiple_environment(
                                a.functionArguments(), environment
                            )
                            argument_symbols.append(
                                (
                                    len(a.functionArguments()),
                                    (a.returns(), new_environment),
                                )
                            )

                        if all(
                            len(new_environment) <= maximum_environment
                            for _, (_, new_environment) in argument_symbols
                        ):
                            rules[(request, environment)].append(
        ...
        eprint(
            sum(len(productions) for productions in rules.values()), "production rules"
        )
    ...
    def __str__(self):
        return f"start symbol: {self.start_symbol}\n\n%s" % (
            "\n\n".join(
                "\n".join(
                    f"{nt} ::= {k}\t%s\t\t{l}" % (" ".join(f"{n}x{s}" for n, s in ar))
                    for l, k, ar in rs
                )
    ...
    def number_rules(self):
        ...
        new_productions = [
            [
                (lp, k, [(nl, mapping[nt]) for nl, nt in arguments])
        ...
        return PCFG(
            new_productions, mapping[self.start_symbol], self.number_of_arguments
        )
                ...
                [
                    {
                        ...
                        "constructor": str(k),
                        "arguments": [
                            {"n_lambda": nl, "nt": nt} for nl, nt in arguments
                        ],
                    }
    ...
    def log_probability(self, program, symbol=None):
        ...
        if not isinstance(program, Program):
            ...
            assert (
                ...
                and 0 <= program < len(self.productions)
                or program in self.productions
            ), f"failure to type production: {program}:{symbol}"
        ...
        for p, k, arguments in rules:
            if f == k:
                _lp = p + sum(
                    self.log_probability(a, at) for a, (_, at) in zip(xs, arguments)
                )
                lp = lse(lp, _lp)
        return lp
    ...
    def best_first_enumeration(self, partial=False):
        ...
        h.push(
            ...
            (
                0.0,
                NamedHole(self.start_symbol).wrap_in_abstractions(
    ...
    def split(self, nc):
        def expansions(expression):
            if isinstance(expression, NamedHole):
                for _, k, arguments in self.productions[expression.name]:
                    arguments = [
                        NamedHole(at).wrap_in_abstractions(nl) for nl, at in arguments
                    ]
                    for a in arguments:
                        k = Application(k, a)
                    yield k
        ...
        initial_split = [
            NamedHole(self.start_symbol).wrap_in_abstractions(self.number_of_arguments)
        ]
        while len(initial_split) < nc:
            biggest = max(initial_split, key=lambda pp: self.log_probability(pp))
            initial_split = list(expansions(biggest)) + [
                pp for pp in initial_split if pp != biggest
            ]
    ...
    def quantized_enumeration(self, resolution=0.5, skeletons=None):
        ...
        if skeletons is None:
            ...
            eprint(skeletons)
        skeleton_costs = [
            int(-self.log_probability(pp) / resolution + 0.5) for pp in skeletons
        ]

        # replace probabilities with quantized costs
        productions = [
            [
                (max(int(-lp / resolution + 0.5), 1), k, arguments)
        ...
        expressions = [
            [None for _ in range(int(100 / resolution))] for _ in range(nonterminals)
        ]
            ...
            if expressions[symbol][size] is None:
                ...
                for cost, k, arguments in productions[symbol]:
                    ...
                    if len(arguments) == 0:
                            ...
                            for a1 in expressions_of_size(at1, c1):
                                ...
                                for c2 in range(size - cost - c1):
                                    for a2 in expressions_of_size(at2, c2):
                                        a2 = a2.wrap_in_abstractions(nl2)
                                        for a3 in expressions_of_size(
                                            at3, size - cost - c1 - c2
                                        ):
                                            a3 = a3.wrap_in_abstractions(nl3)
                                            new.append(
                                                Application(
                                                    Application(Application(k, a1), a2),
                                                    a3,
                                                )
                                            )
                                    ...
                                    for a2 in expressions_of_size(at2, c2):
                                        ...
                                        for c3 in range(size - cost - c1 - c2):
                                            for a3 in expressions_of_size(at3, c3):
                                                a3 = a3.wrap_in_abstractions(nl3)
                                                for a4 in expressions_of_size(
                                                    at3, size - cost - c1 - c2 - c3
                                                ):
                                            ...
                                            for a3 in expressions_of_size(at3, c3):
                                                ...
                                                for c4 in range(
                                                    ...
                                                    for a4 in expressions_of_size(
                                                        ...
                                                        a4 = a4.wrap_in_abstractions(
                                                            nl4
                                                        )
                                                        for a5 in expressions_of_size(
                                                            at3,
                                                            size
                                                            - cost
                                                            - c1
                        ...
                        assert (
                            False
                        ), "more than five arguments not supported for the enumeration algorithm but that is not for any good reason"
