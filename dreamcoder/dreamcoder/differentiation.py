...
class DN(object):
    ...
    def differentiate(self):
        if self.gradient is None:
            self.gradient = sum(
                partial * descendent.differentiate()
    ...
    def zeroEverything(self):
        if (
            self.gradient is None
            and self.descendents == []
            and (self.data is None or self.arguments == [])
        ):
            return
    ...
    def numericallyVerifyGradients(self, parameters):
        ...
        for j, p in enumerate(parameters):
            ...
            if abs(calculatedGradients[j] - d) > 0.1:
                eprint(
                    "Bad gradient: expected %f, got %f" % (d, calculatedGradients[j])
                )
        ...
        for _ in range(attempts):
            ...
            ls.append(
                self.resilientBackPropagation(
                    parameters, lr=lr, steps=steps, decay=decay, grow=grow
                )
            )
        return min(ls)
...
class Placeholder(DN):
    ...
    def __init__(self, initialValue=0.0, name=None):
    ...
    @staticmethod
    def named(namePrefix, initialValue=0.0):
