...
class FragmentGrammar(object):
    ...
    def buildCandidates(self, context, environment, request):
        ...
        for j, t in enumerate(environment):
            try:
                ...
                variableCandidates.append((newContext, t.apply(newContext), Index(j)))
            except UnificationFailure:
                continue
    ...
    def logLikelihood(self, request, expression):
        ...
        if invalid(l):
            ...
            eprint(
                "PANIC: Invalid log likelihood. expression:",
                expression,
                "tp:",
                request,
                "Exported to:",
    ...
    def _logLikelihood(self, context, environment, request, expression):
        ...
        possibleVariables = float(
            int(any(isinstance(candidate, Index) for _, _, _, candidate in candidates))
        )
        possibleUses = {
            candidate: 1.0
        ...
        for f, xs in expression.applicationParses():
            for candidateLikelihood, newContext, tp, production in candidates:
                ...
                if production.isIndex:
                    ...
                    try:
                        newContext, fragmentType, variableBindings = Matcher.match(
                            newContext, production, f, len(xs)
                        )
    ...
    def insideOutside(self, frontiers, pseudoCounts):
        ...
        return FragmentGrammar(
            ...
            - log(max(uses.possibleVariables, 1.0)),
            [
                (
                    log(uses.actualUses.get(p, 0.0) + pseudoCounts)
                    - log(uses.possibleUses.get(p, 0.0) + pseudoCounts),
                    t,
                    p,
                )
    ...
    def jointFrontiersLikelihood(self, frontiers):
        return sum(
            lse(
                [
                    entry.logLikelihood
                    + self.logLikelihood(frontier.task.request, entry.program)
                    for entry in frontier
                ]
            )
            for frontier in frontiers
    ...
    def induceFromFrontiers(
        ...
        if aic is not POSITIVEINFINITY:
            ...
            while True:
                ...
                fragments = [
                    f
                    for f in proposeFragmentsFromFrontiers(
                        restrictedFrontiers, a, CPUs=CPUs
                    )
                ...
                candidateGrammars = [
                    FragmentGrammar.uniform(bestGrammar.primitives + [fragment])
                    for fragment in fragments
                ]
                if not candidateGrammars:
                    break

                scoredFragments = parallelMap(
                    CPUs,
                    grammarScore,
                    candidateGrammars,
                ...
                newPrimitiveLikelihood, newType, newPrimitive = bestGrammar.productions[
                    -1
                ]
                expectedUses = bestGrammar.expectedUses(
                    restrictedFrontiers
                ).actualUses.get(newPrimitive, 0)
                eprint(
                ...
                bestGrammar.productions[-1] = (
                    newPrimitiveLikelihood,
                    concretePrimitive.tp,
                    concretePrimitive,
                )
                frontiers = parallelMap(
                    CPUs,
                    lambda frontier: bestGrammar.rescoreFrontier(
                        RewriteFragments.rewriteFrontier(frontier, newPrimitive)
                    ),
                    frontiers,
                )
                eprint(
                    ...
                    % (
                        bestGrammar.expectedUses(frontiers).actualUses[
                            concretePrimitive
                        ]
                    )
                )
        ...
        productionUses = bestGrammar.expectedUses(
            [f for f in frontiers if not f.empty]
        ).actualUses
        productionUses = {p: productionUses.get(p, 0.0) for p in bestGrammar.primitives}
        possibleUses = bestGrammar.expectedUses(
            [f for f in frontiers if not f.empty]
        ).possibleUses
        possibleUses = {p: possibleUses.get(p, 0.0) for p in bestGrammar.primitives}

        for p in bestGrammar.primitives:
            eprint("%f / %f\t%s" % (productionUses[p], possibleUses[p], p))
        ...
        if False and any(
            productionUses.get(p, 0) < 0.5 for p in grammar.primitives if p.isInvented
        ):
            uselessProductions = [
                p
            ...
            eprint(
                "The following invented primitives are no longer needed, removing them..."
            )
