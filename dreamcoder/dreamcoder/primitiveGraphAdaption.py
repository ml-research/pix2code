...
def graphPrimitives(result, prefix, view=False):
    ...
    primitives = {p for g in result.grammars for p in g.primitives if p.isInvented}
    age = {
        p: min(j for j, g in enumerate(result.grammars) if p in g.primitives) + 1
        for p in primitives
    }
    ...
    def getName(p):
        ...
        for k, childName in children.items():
            simplification_ = simplification_.substitute(
                k, Primitive(childName, None, None)
            )
        for original, simplified in nameSimplification.items():
            simplification_ = simplification_.substitute(
                Primitive(original, None, None), Primitive(simplified, None, None)
            )
        name[p] = "f%d" % len(name)
        simplification[p] = (
            name[p] + "=" + lb(prettyProgram(simplification_, Lisp=True))
        )
    ...
    englishDescriptions = {
        ...
        "#(lambda (unfold $0 (lambda (empty? $0)) (lambda (car $0)) (lambda (#(lambda (lambda (fold $1 $1 (lambda (lambda (cdr (if (char-eq? $1 $2) $3 $0))))))) $0 SPACE))))": "Abbreviate",
        "#(lambda (lambda (fold $1 $1 (lambda (lambda (cdr (if (char-eq? $1 $2) $3 $0)))))))": "Drop until char",
        "#(lambda (lambda (fold $1 $1 (lambda (lambda (if (char-eq? $1 $2) empty (cons $1 $0)))))))": "Take until char",
        "#(lambda (lambda (#(lambda (lambda (fold $0 $1 (lambda (lambda (cons $1 $0)))))) (cons $0 $1))))": "Append char",
        "#(lambda (lambda (map (lambda (if (char-eq? $0 $1) $2 $0)))))": "Substitute char",
        "#(lambda (lambda (length (unfold $1 (lambda (char-eq? (car $0) $1)) (lambda ',') (lambda (cdr $0))))))": "Index of char",
        "#(lambda (#(lambda (lambda (fold $0 $1 (lambda (lambda (cons $1 $0)))))) $0 STRING))": "Append const",
        "#(lambda (lambda (fold $1 $1 (lambda (lambda (fold $0 $0 (lambda (lambda (cdr (if (char-eq? $1 $4) $0 (cons $1 $0)))))))))))": "Last word",
    ...
    # TODO: add english descriptions for our primitives
        "#(lambda (lambda (lambda (index (find $1 $2) (cdr $0)))))": "parse concept",
        "#(lambda (lambda (lambda (eq? (#(lambda (lambda (lambda (index (find $1 $2) (cdr $0))))) $0 $1 $0)": "compare concept",
    }
    ...
    def makeUnorderedGraph(fn):
        ...
        for p in primitives:
            g.node(getName(p), label="<%s>" % simplification[p])
        for p in primitives:
            ...
            with g.subgraph(name="cluster_%d" % o) as sg:
                ...
                for p in ordering[o]:
                    if str(p) in englishDescriptions:
                        thisLabel = (
                            '<<font face="boldfontname"><u>%s</u></font><br />%s>'
                            % (englishDescriptions[str(p)], simplification[p])
                        )
                    else:
    ...
    # return makeGraph(depth2primitives, prefix + "depth.pdf")
    return makeUnorderedGraph(prefix + "unordered.pdf")
