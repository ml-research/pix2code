...
def fragmentSize(f, boundVariableCost=0.1, freeVariableCost=0.01):
    ...
    return (
        leaves + boundVariableCost * boundVariables + freeVariableCost * freeVariables
    )
...
class RewriteFragments(object):
    ...
    def tryRewrite(self, e, numberOfArguments):
        ...
        assert frozenset(bindings.keys()) == frozenset(
            range(len(bindings))
...
def nontrivial(f):
    ...
    return (
        numberOfPrimitives + 0.5 * (numberOfHoles + numberOfVariables) > 1.5
        and numberOfPrimitives >= 1
    )
...
def violatesLaziness(fragment):
    ...
    for surroundingAbstractions, child in fragment.walkUncurried():
        ...
        return any(
            yc.isIndex and yc.i >= yd for yd, yc in y.walk(surroundingAbstractions)
        ) or any(
...
def proposeFragmentsFromProgram(p, arity):
    ...
    return {
        canonicalFragment(f)
        for b in range(arity + 1)
        for f in fragments(p, b)
        if nontrivial(f)
    }
