...
from dreamcoder.utilities import callCompiled, eprint, get_root_dir, timing
...
def induceGrammar(*args, **kwargs):
    ...
    if "pypy" in backend:
        ...
        frontiers = [
            Frontier(f.entries, Task(f.task.name, f.task.request, []))
            for f in frontiers
        ]
        args = [g0, frontiers]
    ...
    if "pypy" in backend:
        ...
        newFrontiers = [
            Frontier(f.entries, original_tasks[f.task.name]) for f in newFrontiers
        ]
...
def memorizeInduce(g, frontiers, **kwargs):
    ...
    newGrammar = Grammar.uniform(
        [p for p in g.primitives] + [Invented(ni) for ni in newInventions]
    )
...
def ocamlInduce(
    ...
    print("Start Compression with CPUs:", CPUs)
    ...
    while True:
        ...
        frontiers = [frontiers.get(f.task, t2f[f.task]) for f in originalFrontiers]
...
def rustInduce(
    ...
    message = {
        ...
        "primitives": [
            {"name": p.name, "tp": str(t), "logp": finite_logp(l)}
            for l, t, p in g0.productions
            if p.isPrimitive
        ],
        "inventions": [
            {"expression": str(p.body), "logp": finite_logp(l)}  # -inf=-100
            for l, t, p in g0.productions
            if p.isInvented
        ],
    ...
    productions = [
        (x["logp"], p)
        for p, x in zip(
            (p for (_, _, p) in g0.productions if p.isPrimitive), resp["primitives"]
        )
    ] + [
    ...
    g = Grammar.fromProductions(
        productions, resp["variable_logprob"], continuationType=g0.continuationType
    )
    newFrontiers = [
        Frontier(
            [
                FrontierEntry(
                    Program.parse(s["expression"]),
                    logPrior=s["logprior"],
