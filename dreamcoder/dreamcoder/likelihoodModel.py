...
from dreamcoder.task import EvaluationTimeout, Task
...
def add_string_constants(tasks):
    for task in tasks:
        task.str_const = longest_common_substr(
    ...
    if ll_cutoff == "gt":
        ...
        for task in tasks:
            ...
            task.gt_test = get_gt_ll(
                task.name, [example[1] for example in regexHeldOutExamples(task)]
            )
        return tasks
    elif ll_cutoff == "plus":
        for task in tasks:
        ...
        for task in tasks:
            ...
            task.ll_cutoff = math.log(
                0.75 * math.exp(biscore) + 0.25 * math.exp(uniscore)
            )
        return tasks
    else:
...
def regex_plus_bound(X):
    ...
    for r in regexes:
        regex_scores.append(
            sum(c[x] * r.match(x) for x in c) / float(sum([len(x) for x in X]))
        )
...
def make_corpus_bigram(C):
    ...
    body_count = Counter(
        element[i : i + 2] for element in str_list for i in range(len(element) - 1)
    )
    ...
    body_logp = {
        x: math.log(
            body_count[x] / body_bigram_n / body_character_n[x[0]] * body_unigram_n
        )
        for x in body_count
    }
...
def bigram_corpus_score(X, logp):
    ...
    for x in X:
        bigram_list = (
            [x[0]] + [x[i : i + 2] for i in range(len(x) - 1)] + [x[-1] + "\n"]
        )
        bigram_list = ["".join(b) if isinstance(b, list) else b for b in bigram_list]

        string_ll = sum(
            logp.get(bigram, float("-inf")) for bigram in bigram_list
        )  # /(len(x) + 1)
...
class ProbabilisticLikelihoodModel:
    ...
    def score(self, program, task):
        ...
        try:
            ...
            normalized_cum_ll = cum_ll / float(
                sum([len(example) for example in example_list])
            )
        ...
        def train(self, tasks, steps=400):
            ...
            with timing("Trained discriminator"):
                ...
                for i in range(steps):
                    ...
                    if not i % 50:
                        eprint(
                            "Discriminator Epoch", i, "Loss", sum(losses) / len(losses)
                        )
                        gc.collect()
