...
def main(opt):
    ...
    if val_loader is not None:
        meters["val"] = {
            field: tnt.meter.AverageValueMeter() for field in opt["log.fields"]
        }
    ...
    def on_start(state):
        ...
        state["scheduler"] = lr_scheduler.StepLR(
            state["optimizer"], opt["train.decay_every"], gamma=0.5
        )
    ...
    def on_end_epoch(hook_state, state):
        ...
        if val_loader is not None:
            if meter_vals["val"]["loss"] < hook_state["best_loss"]:
                ...
                state["model"].cpu()
                torch.save(
                    state["model"], os.path.join(opt["log.exp_dir"], "best_model.pt")
                )
                if opt["data.cuda"]:
                    state["model"].cuda()
