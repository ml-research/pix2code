...
class Image_RobustFill(nn.Module):
    def __init__(
        ...
        # image encoder:
        self.conv1 = nn.Conv2d(3, 8, kernel_size=(3, 3), padding=(3, 3), stride=(1, 1))
        self.conv2 = nn.Conv2d(8, 16, kernel_size=(3, 3), padding=(1, 1), stride=(1, 1))
        self.conv3 = nn.Conv2d(
            16, 16, kernel_size=(3, 3), padding=(1, 1), stride=(1, 1)
        )
    ...
    def score(self, batch_inputs, batch_target, autograd=False):
        ...
        inputs = torch.stack(
            tuple(torch.tensor(b) for b in batch_inputs), dim=0
        ).float()
    ...
    def _run(self, inputs, target=None, mode="sample", n_samples=None):
        ...
        if self.no_inputs:
            ...
            max_length_inputs = [
                [3 * 3 for j in range(n_examples)] for i in range(self.n_encoders)
            ]  # TODO
        ...
        def attend(i, j, h):
            ...
            theta = torch.cat(
                (F.relu(self.h_to_32_linear(h)), F.relu(self.img_to_32(linear_img))), 1
            )  # right
            ...
            linear_transformed_img = transformed_img.view(
                -1,
                transformed_img.size(1)
                * transformed_img.size(2)
...
if __name__ == "__main__":
    ...
    omni_transforms = transforms.Compose(
        [resizing, transforms.ToTensor(), rescaling, flip]
    )  # TODO: check this, but i think i don't want rescaling

    kwargs = {"num_workers": 1, "pin_memory": True, "drop_last": True}
    train_loader = torch.utils.data.DataLoader(
        datasets.Omniglot(
            "../vhe/data", download=True, background=True, transform=omni_transforms
        ),
    ...
    test_loader = torch.utils.data.DataLoader(
        datasets.Omniglot(
            "../vhe/data", download=True, background=False, transform=omni_transforms
        ),
        batch_size=batch_size,
        shuffle=True,
        **kwargs
    ...
    for i, batch in enumerate(train_loader):
        ...
        if i % 10 == 0:
            print(
                "Iteration %d" % (i),
                "Score %3.3f" % score,
                "(%3.3f seconds per iteration)" % ((time.time() - start) / (i + 1)),
            )
