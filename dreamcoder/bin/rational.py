...
from dreamcoder.program import Abstraction, Application, Primitive
...
def drawFunction(n, dx, f, resolution=64):
    ...
    figure = plot.figure()
    plot.plot(
        np.arange(-dx, dx, 0.05), [f(x) for x in np.arange(-dx, dx, 0.05)], linewidth=20
    )
    plot.ylim([-10, 10])
    plot.axis("off")
    figure.canvas.draw()
    ...
    data = np.array(
        ...
        .resize(size=(resolution, resolution), resample=Image.BICUBIC)
        .getdata()
...
def rational_options(p):
    p.add_argument(
        "--smooth", action="store_true", default=False, help="smooth likelihood model"
    )
