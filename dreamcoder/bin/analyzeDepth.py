...
def alignedPermutation(*parallel):
    ...
    p = list(range(n))
    random.shuffle(p)
    return tuple([x[i] for i in p] for x in parallel)
...
if __name__ == "__main__":
    ...
    if arguments.every:
        ...
        for mode in ["MEAN", "SIZE", "MAX"]:
                ...
                for domain, data in results.items():
                    for i, y, d in data:
                        ...
                        if results is fullResults:
                            # 003f5c
                            c = (0.0, factor * 63.0 / 92.0, factor * 92.0 / 92.0)
            ...
            plot.ylabel("% Test Solved", fontsize=FONTSIZE)
            plot.xlabel(
                {"MAX": "Max depth", "MEAN": "Avg. depth", "SIZE": "Library size"}[
                    mode
                ],
                fontsize=FONTSIZE,
            )
            ...
            plot.savefig(
                f"figures/depthVersusAccuracy_revision_{mode}.png", bbox_inches="tight"
            )
    ...
    for mode in ["MAX", "MEAN", "SIZE"]:
        ...
        for domain in set(fullResults.keys()) | set(lesionResults.keys()):
            if domain in fullResults:
                scatter(fullResults[domain], colors[0], MARKERONE)
            if domain in lesionResults:
                scatter(lesionResults[domain], colors[0], MARKERTWO)
            legend.append((domain, colors[0]))
            colors = colors[1:]

        plot.ylabel("% Test Solved", fontsize=FONTSIZE)
        plot.xlabel(
            {"MAX": "Max depth", "MEAN": "Avg. depth", "SIZE": "Library size"}[mode],
            fontsize=FONTSIZE,
        )
        plot.xticks(fontsize=TICKFONTSIZE)
        plot.yticks(fontsize=TICKFONTSIZE)
        ...
        handles = [
            mlines.Line2D([], [], color="k", marker=marker, ls="None", label=label)
            for marker, label in [(MARKERONE, "Full model"), (MARKERTWO, "No Rec")]
        ]
        if arguments.legend:
            handles = []
        handles.extend(
            [
                mlines.Line2D([], [], color=color, marker="o", ls="None", label=label)
        ...
        plot.savefig(
            f"figures/depthVersusAccuracy_mainPaper_{mode}.eps", bbox_inches="tight"
        )
        plot.figure()
        legend = plot.legend(
            handles=handles,
        ...
        f.savefig("figures/depthVersusAccuracy_mainPaper_legend.eps", bbox_inches=bb)
