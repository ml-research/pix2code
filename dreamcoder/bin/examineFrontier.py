...
def prettyRegex(preg):
    return (
        preg.str()
        .replace("[ABCDEFGHIJKLMNOPQRSTUVWXYZ]", "\\u")
        .replace("[0123456789]", "\\d")
        .replace("[abcdefghijklmnopqrstuvwxyz]", "\\l")
        .replace(
...
if __name__ == "__main__":
    ...
    for task in tasks:
        ...
        eprint(
            verbatimTable(
                ["".join(example[1]) for example in task.examples]
                + [None]
                + [gt_preg, None]
        ...
        def examineProgram(entry):
            ...
            if autodiff:
                ...
                for i in range(AUTODIFF_ITER):
                    ...
                    score = sum(
                        preg.match("".join(example[1])) for example in task.examples
                    )
            ...
            testing_likelihood = sum(
                preg.match(testingString) for _, testingString in testingExamples
            )
            ground_truth_testing = sum(
                gt_preg.match(testingString) for _, testingString in testingExamples
            )
            ...
            if testing_likelihood >= ground_truth_testing:
                print(
                    f"\t HIT (test), Ground truth: {ground_truth_testing}, found ll: {testing_likelihood}"
                )
            else:
                print(
                    f"\t MISS (test), Ground truth: {ground_truth_testing}, found ll: {testing_likelihood}"
                )
        ...
        programSamples = [
            p.visit(ConstantVisitor(task.str_const)).evaluate([])(pre.String(""))
        ...
        posterior = [
            (
                e.logPosterior,
                e.program.visit(ConstantVisitor(task.str_const)).evaluate([])(
                    pre.String("")
                ),
            )
        ...
        testingLikelihood = lse(
            [lp + sum(r.match(te) for te in testingExamples) for lp, r in posterior]
        )
        trainingExamples = ["".join(example[1]) for example in task.examples]
        print("trainingExamples", trainingExamples)
        trainingLikelihood = lse(
            [lp + sum(r.match(te) for te in trainingExamples) for lp, r in posterior]
        )
    ...
    print(
        f"Best posteriorc hits training task {posteriorHits}/{totalTasks} = {posteriorHits/totalTasks}"
    )
    print(
        f"Best likelihood hits training task {likelihoodHits}/{totalTasks} = {likelihoodHits/totalTasks}"
    )

    print(
        f"Best posteriorc hits testing task {posteriorHits_test}/{totalTasks} = {posteriorHits_test/totalTasks}"
    )
    print(
        f"Best likelihood hits testing task {likelihoodHits_test}/{totalTasks} = {likelihoodHits_test/totalTasks}"
    )

    print(
        f"Posterior predictive hits training task {marginalHits}/{totalTasks} = {marginalHits/totalTasks}"
    )
    print(
        f"Posterior predictive hits testing task {marginalHits_test}/{totalTasks} = {marginalHits_test/totalTasks}"
    )
