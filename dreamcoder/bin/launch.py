...
def launchAmazonCloud(size="t2.micro", name=""):
    ...
    os.system(
        "aws ec2 create-tags --resources %s --tags Key=Name,Value=%s" % (instance, name)
    )
    os.system(
        """
...
def sendCommand(
    address, script, job_id, upload, ssh_key, resume, tar, shutdown, checkpoint=None
):
    import tempfile
...
def launchExperiment(
    ...
    if resume and "resume" not in command:
        print(
            "You said to resume, but didn't give --resume to your python command. I am assuming this is a mistake."
        )
        sys.exit(1)
...
""" % (
    ...
    if arguments.google:
        ...
        instance, address = launchAzureCloud(
            size, name=name, region=arguments.azure_region
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resume", metavar="TAR", type=str, help="send tarball to resume checkpoint."
    )
    parser.add_argument(
        "--checkpoint",
        metavar="checkpoint",
    ...
    parser.add_argument(
        "--ssh_key", default="id_rsa", help="Name of local RSA key file for openmind."
    )
