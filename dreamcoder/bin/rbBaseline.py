...
from rational import FeatureExtractor, RandomParameterization, drawFunction
...
from dreamcoder.domains.arithmetic.arithmeticPrimitives import (
    real, real_addition, real_division, real_multiplication)
...
from dreamcoder.domains.list.makeListTasks import (EASYLISTTASKS,
...
from dreamcoder.domains.regex.makeRegexTasks import (  # , add_string_constants
    makeHandPickedTasks, makeLongTasks, makeNewNumberTasks, makeNewTasks,
    makeNumberTasks, makeOldTasks, makeShortTasks, makeWordTasks)
...
from dreamcoder.type import (Context, UnificationFailure, arrow, t0, tbool,
                             tint, tlist, tpregex)
    ...
    list(
        map(
            ...
            list(
                {
                    tuple([c for c in s])
                    for t in loadPBETasks("PBE_Strings_Track")[0]
                    + dreamcoder.domains.text.makeTextTasks.makeTasks()
...
def makeExamples(task):
    ...
    if arguments.domain == "regex":
        return [([], y) for xs, y in task.examples]
    ...
    if arguments.domain == "logo":
        i = np.array([float(xx) / 256.0 for xx in task.highresolution])
        i = i.reshape((128, 128))
...
def test_task(m, task, timeout):
        ...
        for cand in candidates:
            ...
            if p not in failed_cands:
                if "STRING" in str(p):
                    ...
                    if len(task.stringConstants) == 0:
                        ...
                        ll = min(
                            task.logLikelihood(
                                pp,
                                timeout=0.1 if arguments.domain != "rational" else None,
                            )
                ...
                if arguments.domain == "regex":
                    ...
                    ll = float("-inf")
                    if not p.canHaveType(task.request):
                        p = None
                    else:
                        ...
                        try:
                            ...
                            if arguments.sampleLikelihood:
                                ...
                                dataLikelihood = sum(
                                    regex.match("".join(y)) for _, y in task.examples
                                )
                    ...
                    try:
                        yh = drawLogo(p, timeout=1.0, resolution=28)
                        if (
                            isinstance(yh, list)
                            and list(map(int, yh)) == task.examples[0][1]
                        ):
                            ll = 0.0
                        else:
                            ll = float("-inf")
    ...
    print(
        "for this task I think that the following is the map estimate:\n",
        frontier.topK(1),
    )
    if arguments.taskLikelihood:
        return lse([e.logPrior + e.logLikelihood for e in frontier])
    return lse(
        [e.logPosterior + testingRegexLikelihood(task, e.program) for e in frontier]
    )
...
if __name__ == "__main__":
    ...
    if arguments.domain == "text":
        ...
        g = Grammar.uniform(reducedConcatPrimitives(), continuationType=tpregex)
        tasks = makeNewTasks()
        ...
        g = Grammar.uniform(
            dreamcoder.domains.logo.logoPrimitives.primitives, continuationType=turtle
        )
    ...
    if not arguments.test:
        ...
        if arguments.domain in ["tower", "rational", "logo"]:
            ...
            m = SyntaxCheckingRobustFill(
                input_vocabularies=input_vocabularies,
        ...
        for i in range(max_n_iterations):
            ...
            if i % 10 == 0:
                print(
                    f"Iteration {i}/{max_n_iterations}, Score {score}, ({(time.time()-start)/(i+1)} seconds per iteration)",
                    flush=True,
                )
        ...
        if arguments.domain == "regex":
            from dreamcoder.domains.regex.makeRegexTasks import \
