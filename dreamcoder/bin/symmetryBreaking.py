...
def main():
    trainingTimeout = 600
    rt = arrow(tint, tint)
    g = Grammar.uniform([Program.parse(p) for p in ["1", "0", "+"]])
    tasks = [Task("dummy", rt, [(tuple([n]), 9) for n in range(3)])]
    hf = backgroundHelmholtzEnumeration(tasks, g, 60, evaluationTimeout=0.001)
    hf = hf()
    ...
    for contextual in [True, False]:
        for trainingSource in ["random", "enumerated"]:
            ...
            r.train(
                [],
                helmholtzFrontiers=hf if trainingSource == "enumerated" else [],
                helmholtzRatio=1.0,
                defaultRequest=rt,
