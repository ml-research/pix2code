...
def getExperimentNames(experimentsFile):
    ...
    for line in lines:
        if "*" in line:
            name = line[line.index("*") :].split()[0][
                1:
            ]  # Split from * to first space, then remove asterisk.
...
def getMostRecent(ex1, ex2):
    ...
    date1, date2 = datetime.strptime(date1, dateFormat), datetime.strptime(
        date2, dateFormat
    )
...
def parseLogFile(experimentLog, logsDirectory):
    ...
    for i, line in enumerate(lines):
        testing_iteration = re.match(
            "Evaluating on held out testing tasks for iteration: (\d+)", line
        )
    ...
    is_done = (
        "DONE" if (testingStats["test_iteration"][-1] == total_iterations - 1) else ""
    )
    last_few_test_hits = (
        testingStats["test_hits"][-1],
        testingStats["test_hits"][-2] if len(testingStats["test_hits"]) > 2 else -1,
        testingStats["test_hits"][-3] if len(testingStats["test_hits"]) > 3 else -1,
    )
    print(
        "%s %s Total iterations: %d, on test iteration %d, best test is %d/%d, last few tests are: %d, %d, %d"
        % (
            is_done,
            possiblyBroken,
            total_iterations,
...
if __name__ == "__main__":
    ...
    summarizeLogs(arguments.experimentsFile, arguments.logsDirectory)
