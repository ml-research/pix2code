...
labeledLogos = {
    ...
    "Greek spiral slanted by 2pi/6",
    "Greek spiral 8",
    "row of 5 dashes",
    "row of 6 semicircles",
    "row of 3 circles",
...
def exportTaskTimes(resultPaths, experimentNames, timesArg, export):
    ...
    for j, path in enumerate(resultPaths):
        ...
        # Get all the times.
        tasks = [
            t for t in recognitionTaskMetrics if timesArg in recognitionTaskMetrics[t]
        ]
        ...
        taskTimes = [
            time if time is not None else enumerationTimeout for time in taskTimes
        ]
...
def plotTimeMetrics(
    resultPaths, experimentNames, outlierThreshold, metricsToPlot, timesArg, export=None
):
    ...
    for j, path in enumerate(resultPaths):
        ...
        for k, metricToPlot in enumerate(metricsToPlot):
            ...
            if outlierThreshold:
                ...
                taskNames, taskTimes, taskMetrics = (
                    noOutliersNames,
                    noOutliersTimes,
                    noOutliersMetrics,
                )
            ...
            if outlierThreshold:
                xlabel = "Recognition Best Times, Outlier Threshold: %d" % (
                    outlierThreshold
                )
            else:
                xlabel = "Recognition Best Times"
            title = "Experiment: %s Domain: %s, Iteration: %d" % (
                experimentName,
                domain,
                iterations,
            )
...
def plotEmbeddingWithLabels(
    embeddings, labels, title, exportPath, xlabel=None, ylabel=None, colorLabeling=None
):
    ...
    if colorLabeling is "text":
        ...
        patches = [
            mpatches.Patch(color=cmap(i), label=textTaskPrettyNames[i])
            for i in range(len(textTaskPrettyNames))
        ]
        legend = plot.legend(
            handles=patches,
            frameon=True,
    ...
    if colorLabeling is "list":
        ...
        patches = [
            mpatches.Patch(color=cmap(i), label=prettyNames[i])
...
def plotLabeledImages(
    ...
    colorLabels = [
        cl for ti, cl in enumerate(colorLabels) if colorLabels[ti] is not None
    ]
    ...
    if len(prettyNames) <= 10:
        ...
        colors = [
            cmap(i)
            for i in (list(range(0, 22, 2)) + list(range(1, 22, 2)))[: len(prettyNames)]
        ]
    else:
        assert (
            False
        ), "I don't know what colormap to use when you have more than twenty clusters!"

    import matplotlib.patches as mpatches

    patches = [
        mpatches.Patch(color=colors[i], label=prettyNames[i])
        for i in range(len(prettyNames))
    ]
    legend = plot.legend(
        handles=patches,
    ...
    def trimImage(image):
        image = 255.0 - image[:, :, 0]
        while image[0, :].sum() == 0.0:
            image = image[1:, :]
        while image[-1, :].sum() == 0.0:
            image = image[:-1, :]
        while image[:, 0].sum() == 0.0:
            image = image[:, 1:]
        while image[:, -1].sum() == 0.0:
        ...
        Alpha = 255 * (image > 0)
        return np.dstack([255.0 - image] * 3 + [Alpha])
    ...
    for index, (x, y, i) in enumerate(imageLabels):
        ab = AnnotationBbox(
            OffsetImage(i, zoom=zoom), (x, y), xycoords="data", frameon=False
        )
...
def makeTowerImage(im, labelsAndImages):
    ...
    if labelsAndImages:  # trim
        ...
        while im[0, :, -1].sum() == 0.0:
            im = im[1:, :, :]
        while im[-1, :, -1].sum() == 0.0:
            im = im[:-1, :, :]
        while im[:, 0, -1].sum() == 0.0:
            im = im[:, 1:, :]
        while im[:, -1, -1].sum() == 0.0:
...
def frontierMetric(frontiers):
    primitives = {
        p for f in frontiers for e in f.entries for p in [e.program] if p.isInvented
    }
...
def formattedName(metricToCluster, item):
    ...
    raw_name = raw_name.replace("lambda", "λ")
    if (len(raw_name)) > 100:
        return raw_name[:50] + "\n" + raw_name[50:]
    else:
        return raw_name
...
def random_restart_tsne(X, perplexity, lr, restarts=30):
    with timing(f"did {restarts} random restarts"):
        ...
        for _ in range(restarts):
            tsne = TSNE(
                random_state=0, perplexity=perplexity, learning_rate=lr, n_iter=10000
            )
...
def plotTSNE(
    ...
    for j, path in enumerate(resultPaths):
        ...
        for k, metricToCluster in enumerate(metricsToCluster):
            ...
            for task in sorted(
                recognitionTaskMetrics.keys(),
                key=lambda task: formattedName(metricToCluster, task),
            ):
                if metricToCluster in recognitionTaskMetrics[task]:
                    ...
                    if recognitionTaskMetrics[task][metricToCluster] is not None:
                        ...
                        taskMetrics.append(
                            recognitionTaskMetrics[task][metricToCluster]
                        )
            if metricToCluster == "frontier":
                taskMetrics = [
                    f.expectedProductionUses(result.grammars[-1]) for f in taskMetrics
                ]
            ...
            print(
                "Clustering %d tasks with embeddings of shape: %s"
                % (len(taskMetrics), str(taskMetrics[0].shape))
            )
            if taskMetrics[0].shape[0] == 0:
                print(
                    f"Task metrics have zero dimensionality - skipping {metricToCluster}/{iterations}"
                )
                continue
            ...
            taskMetrics = taskMetrics / np.reshape(
                metricNorms, (metricNorms.shape[0], 1)
            )
            print(taskNames.shape, taskMetrics.shape)

            clusteredTaskMetrics = random_restart_tsne(
                taskMetrics, perplexity=tsnePerplexity, lr=tsneLearningRate, restarts=10
            )
            title = title or (
                "Metric: %s, Domain: %s, Experiment: %s, Iteration: %d"
                % (metricToCluster, domain, experimentName, iterations)
            )
            ...
            if labelWithImages or labelsAndImages:
                ...
                for i, task in enumerate(
                    sorted(
                        filter(
                            lambda mt: isinstance(mt, Task),
                            recognitionTaskMetrics.keys(),
                        ),
                        key=lambda task: task.name,
                    )
                ):  # Enumerate in same order as sorted tasks.
                    if domain == "tower":
                        recognitionTaskMetrics[task]["taskImages"] = task.getImage(
                            pretty=not labelsAndImages
                        )
                    if (
                        "taskImages" not in recognitionTaskMetrics[task]
                        and domain == "rational"
                    ):
                        recognitionTaskMetrics[task]["taskImages"] = task.features
                    if (
                        "taskImages" not in recognitionTaskMetrics[task]
                        and domain == "logo"
                    ):
                        recognitionTaskMetrics[task]["taskImages"] = task.highresolution
                ...
                if not labelsAndImages:
                    plotEmbeddingWithImages(
                        clusteredTaskMetrics,
...
def getGroundTruthStarts(groundTruthCheckpoints):
    ...
    def getFrontierStart(frontiers):
        ...
        topEntry = sorted(
            frontiers.entries, key=lambda e: e.logLikelihood, reverse=True
        )[0]
        p = topEntry.program
        while p.isAbstraction:
            p = p.body
        while p.isApplication:
            p = p.f
        return p
    ...
    for j, path in enumerate(groundTruthCheckpoints):
        ...
        trainToStarts = {
            task: getFrontierStart(frontiers)
            for task, frontiers in taskSolutions.items()
            if len(frontiers) > 0
        }
        ...
        for task in sorted(
            recognitionTaskMetrics.keys(),
            key=lambda task: formattedName("frontier", task),
        ):
            if "frontier" in recognitionTaskMetrics[task] and task not in trainToStarts:
                if len(recognitionTaskMetrics[task]["frontier"]) > 0:
                    testToStarts[task] = getFrontierStart(
                        recognitionTaskMetrics[task]["frontier"]
                    )
        startToTests = invMap(testToStarts)
        frontierStarts.append(
            [trainToStarts, startToTrains, testToStarts, startToTests]
        )
...
def DPGMM(metricsDict, groundTruths):
    ...
    dpgmm = mixture.BayesianGaussianMixture(
        n_components=len(groundTruths), covariance_type="full"
    ).fit(taskMetrics)
    ...
    for j, path in enumerate(checkpoints):
        ...
        for task in sorted(
            ...
            if "expectedProductionUses" in recognitionTaskMetrics[task]:
                trainExpectedProductions[task] = recognitionTaskMetrics[task][
                    "expectedProductionUses"
                ]
            elif "frontier" in recognitionTaskMetrics[task] and task in testToStarts:
                testExpectedProductions[task] = recognitionTaskMetrics[task][
                    "frontier"
                ].expectedProductionUses(result.grammars[-1])

        allEPs.append(
            [
                DPGMM(trainExpectedProductions, startToTrains),
                DPGMM(testExpectedProductions, startToTests),
            ]
        )
    return allEPs
...
def plotHeatMap(
    cm, classes, title="Confusion matrix", cmap=plot.cm.Blues, exportPath=None
):
    """
...
def getClusterIntersections(allMetricClusters):
    ...
    trainTaskToClusters = [
        trainDict
        for (trainKeys, _, trainDict), (testKeys, _, testDict) in allMetricClusters
    ]
    testTaskToClusters = [
        testDict
        for (trainKeys, _, trainDict), (testKeys, _, testDict) in allMetricClusters
...
def clusteringAnalysis(
    ...
    if compareToExpectedProductionUses:
        groundTruthEPs = getExpectedProductionUses(
            groundTruthCheckpoints, groundTruthStarts
        )
    ...
    for metric in clusteringMetrics:
        if metric in metricToHeldout:
            ...
            heldoutMetric = "None"
        print(
            "Clustering on metric: "
            + metric
            + " heldoutMetric: "
            + metricToHeldout[metric]
        )
        allMetricClusters = []
        for j, path in enumerate(checkpoints):
            result, domain, iterations, recognitionTaskMetrics = loadResult(path)
            ...
            for task in sorted(
                ...
                if metric in recognitionTaskMetrics[task] and task in trainToStarts:
                    ...
                    heldoutMetric in recognitionTaskMetrics[task]
                    and task in testToStarts
                ):
            ...
            allMetricClusters.append(
                [DPGMM(trainMetrics, startToTrains), DPGMM(testMetrics, startToTests)]
            )
        ...
        if compareToGroundTruthStarts:
            ...
            for j, path in enumerate(checkpoints):
                ...
                trainGroundTruthLabels = preprocessing.LabelEncoder().fit_transform(
                    [str(val) for val in trainToStarts.values()]
                )
                groundTruthStartsRITrain.append(
                    adjusted_rand_score(trainGroundTruthLabels, trainLabels)
                )

                if testLabels:
                    testGroundTruthLabels = preprocessing.LabelEncoder().fit_transform(
                        [str(val) for val in testToStarts.values()]
                    )
                    groundTruthStartsRITest.append(
                        adjusted_rand_score(testGroundTruthLabels, testLabels)
                    )

            print(
                "Comparison to ground truth starts RI train: %s"
                % str(groundTruthStartsRITrain)
            )
            print(
                "Comparison to ground truth starts RI test: %s"
                % str(groundTruthStartsRITest)
            )
        ...
        if compareToExpectedProductionUses:
            ...
            for j, path in enumerate(checkpoints):
                ...
                (_, trainLabels, _), (_, testLabels, _) = allMetricClusters[j]
                groundTruthEPTrain.append(
                    adjusted_rand_score(trainTrueLabels, trainLabels)
                )
                if testLabels:
                    groundTruthEPTest.append(
                        adjusted_rand_score(testTrueLabels, testLabels)
                    )
            print(
                "Comparison to ground truth EP RI train: %s" % str(groundTruthEPTrain)
            )
...
def getExpectedProductionSimilarities(checkpoints, ordering, applySoftmax):
    ...
    for j, path in enumerate(checkpoints):
        ...
        trainTasks = [
            task
            for task, frontiers in result.taskSolutions.items()
            if len(frontiers) > 0
        ]
        ...
        for task in sorted(
            ...
            if "expectedProductionUses" in recognitionTaskMetrics[task]:
                ...
                trainExpected.append(
                    recognitionTaskMetrics[task]["expectedProductionUses"]
                )
            elif "frontier" in recognitionTaskMetrics[task] and task not in trainTasks:
                testNames.append(formattedName("frontier", task))
                testExpected.append(
                    recognitionTaskMetrics[task]["frontier"].expectedProductionUses(
                        result.grammars[-1]
                    )
                )

        trainNames, trainExpected, testNames, testExpected = (
            np.array(trainNames),
            np.array(trainExpected),
            np.array(testNames),
            np.array(testExpected),
        )
        if ordering == "MDS":
            print("Fitting an MDS ordering.")
            # Fit an MDS to one component and use to sort.
            trainOrdering, testOrdering = getMDSOrdering(trainExpected), getMDSOrdering(
                testExpected
            )
            trainNames, trainExpected, testNames, testExpected = (
                trainNames[trainOrdering],
                trainExpected[trainOrdering],
                testNames[testOrdering],
                testExpected[testOrdering],
            )
        ...
        allEPs.append(
            [
                (
                    trainNames,
                    trainExpected,
                    getSimilarities(trainExpected, applySoftmax),
                ),
                (testNames, testExpected, getSimilarities(testExpected, applySoftmax)),
            ]
        )

        print(
            "Found %d ground truth train and %d test."
            % (len(trainExpected), len(testExpected))
        )
    return allEPs


def getSimilarityMetrics(
    similarityMetric, recognitionTaskMetrics, trainNames, testNames, applySoftmax
):
    newRecognitionMetrics = {
        formattedName("frontier", task): value
        for task, value in recognitionTaskMetrics.items()
    }
    trainMetrics = [
        newRecognitionMetrics[taskName][similarityMetric]
        for taskName in trainNames
        if similarityMetric in newRecognitionMetrics[taskName]
    ]
    testMetrics = []
    if similarityMetric in metricToHeldout:
        heldoutMetric = metricToHeldout[similarityMetric]
        testMetrics = [
            newRecognitionMetrics[taskName][heldoutMetric]
            for taskName in testNames
            if heldoutMetric in newRecognitionMetrics[taskName]
        ]
    if applySoftmax:
    ...
    return (trainMetrics, getSimilarities(trainMetrics, applySoftmax)), (
        testMetrics,
        getSimilarities(testMetrics, applySoftmax),
    )
...
def groundTruthStartsToLabels(taskToStarts, taskNames):
    newTaskToStarts = {
        formattedName("frontier", task): str(value)
        for task, value in taskToStarts.items()
    }
...
def logo_tower_NamesToLabels(listNames, nameMapping):
    ...
    labels = [
        prettyNames.index(label) if label in prettyNames else None for label in labels
    ]
    return labels


def getTopNMostSimilar(names, sims, topN):
    sortedSims = np.dstack(
        np.unravel_index(np.argsort(-sims.ravel()), sims.shape)
    ).squeeze()
    for n in range(topN):
...
def getTaskSimilarities(
    taskName, names, sims, trueSims, recognitionTaskMetrics, exportPath, topN=5
):
...
def saveImageMontage(taskNames, recognitionTaskMetrics, exportPath):
    ...
    images = [
        makeLogoImage(np.array(newRecognitionMetrics[task]["taskImages"]))
...
def similarityAnalysis(
    ...
    groundTruthSimilarities = getExpectedProductionSimilarities(
        groundTruthCheckpoints, ordering, applySoftmax
    )
    groundTruthStarts = getGroundTruthStarts(groundTruthCheckpoints)
    ...
    for j, path in enumerate(checkpoints):
        result, domain, iterations, recognitionTaskMetrics = loadResult(path)
        (
            (trainNames, trainExpected, trainTruthSims),
            (testNames, testExpected, testTruthSims),
        ) = groundTruthSimilarities[j]
        trainToStarts, startToTrains, testToStarts, startToTests = groundTruthStarts[j]
        ...
        for similarityMetric in similarityMetrics:
            ...
            if similarityWithTSNE:
                ...
                tsne = TSNE(
                    random_state=0, perplexity=30, learning_rate=400, n_iter=10000
                )
                clusteredTrainMetrics = tsne.fit_transform(trainMetrics)
                plotEmbeddingWithLabels(
                    clusteredTrainMetrics,
                    trainNames,
                    similarityMetric,
                    os.path.join(
                        exportPath,
                        experimentNames[j]
                        + "_it_%d_%s_train_tsne.png" % (iterations, similarityMetric),
                    ),
                    colorLabeling="list",
                )
            #       if testMetrics:
...
def clusterPrograms(checkpoint, distanceThreshold=3.0):
    ...
    data = [
        (t.name, metrics["frontier"].expectedProductionUses(g))
        for t, metrics in checkpoint.recognitionTaskMetrics.items()
    ...
    clustering = AgglomerativeClustering(
        linkage="ward", n_clusters=None, distance_threshold=distanceThreshold
    ).fit(X)
    clusters = {}
...
if __name__ == "__main__":
    ...
    # Clustering analysis
    parser.add_argument(
        "--clusteringAnalysisMetrics", nargs="+", type=str, default=None
    )
    parser.add_argument("--groundTruthCheckpoints", nargs="+", default=None)
    parser.add_argument("--clusteringMethod", type=str, default="dpgmm")
    parser.add_argument(
        "--compareToGroundTruthStarts", default=False, action="store_true"
    )
    parser.add_argument(
        "--compareToExpectedProductionUses", default=False, action="store_true"
    )

    # Similarity matrix analysis
    parser.add_argument(
        "--similarityAnalysisMetrics", nargs="+", type=str, default=None
    )
    ...
    parser.add_argument("--distanceThreshold", type=float, default=3.0)

    parser.add_argument("--export", "-e", type=str, default="data")
