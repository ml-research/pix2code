...
def choose(matrix, idxs):
    if type(idxs) is Variable:
        idxs = idxs.data
    assert matrix.ndimension() == 2
    unrolled_idxs = idxs + torch.arange(0, matrix.size(0)).type_as(idxs) * matrix.size(
        1
    )
...
class SyntaxCheckingRobustFill(nn.Module):
    def __init__(
        ...
        self.v_inputs = [
            len(x) for x in input_vocabularies
        ...
        if cell_type == "GRU":
            ...
            self.decoder_cell = nn.GRUCell(
                input_size=self.v_target + 1, hidden_size=self.hidden_size, bias=True
            )

            self.syntax_decoder_cell = nn.GRUCell(
                input_size=self.v_target + 1, hidden_size=self.hidden_size, bias=True
            )
        ...
        if cell_type == "LSTM":
            ...
            for i in range(self.n_encoders):
                ...
                self.encoder_cells.append(
                    nn.LSTMCell(
                        input_size=input_size, hidden_size=self.hidden_size, bias=True
                    )
                )
            ...
            self.syntax_decoder_cell = nn.LSTMCell(
                input_size=self.v_target + 1, hidden_size=self.hidden_size, bias=True
            )
    ...
    def with_target_vocabulary(self, target_vocabulary):
        ...
        """
        print(
            "WARNING: with_target_vocabulary not yet tested for syntax checker RobsutFill"
        )
        # assert False
        ...
        for i in range(len(target_vocabulary)):
            if target_vocabulary[i] in self.target_vocabulary:
                ...
                syntax_decoder_ih.append(
                    self.syntax_decoder_cell.weight_ih.data[:, j : j + 1]
                )
                ...
                syntax_decoder_ih.append(
                    self._zeros(self.syntax_decoder_cell.weight_ih.data.size(0), 1)
                )
        ...
        self.syntax_decoder_cell.input_size = (
            self.syntax_decoder_cell.weight_ih.data.size(1)
        )
    ...
    def optimiser_step(self, batch_inputs, batch_target, vocab_filter=None):
        ...
        if not hasattr(self, "opt"):
            self._get_optimiser()
        self.opt.zero_grad()
        score, syntax_score = self.score(
            batch_inputs, batch_target, autograd=True, vocab_filter=vocab_filter
        )
        score = score.mean()
    ...
    def score(self, batch_inputs, batch_target, autograd=False, vocab_filter=None):
        ...
        _, score, syntax_score = self._run(
            inputs, target=target, mode="score", vocab_filter=vocab_filter
        )
        if autograd:
    ...
    def sample(self, batch_inputs=None, n_samples=None, vocab_filter=None):
        ...
        inputs = self._inputsToTensors(batch_inputs)
        target, score, syntax_score = self._run(
            inputs, mode="sample", n_samples=n_samples, vocab_filter=vocab_filter
        )
    ...
    def _refreshVocabularyIndex(self):  # TODO
        ...
        self.target_vocabulary_index = {
            self.target_vocabulary[j]: j for j in range(len(self.target_vocabulary))
        }

    def __getstate__(self):
        if hasattr(self, "opt"):
            return dict(
                [(k, v) for k, v in self.__dict__.items() if k is not "opt"]
    ...
    def _syntax_decoder_get_init(self, h=None, batch_size=None):
        ...
        if self.cell_type == "GRU":
            return h
        if self.cell_type == "LSTM":
            return (h, self.syntax_decoder_init_c.repeat(h.size(0), 1))  # TODO
    ...
    def _run(
        self, inputs, target=None, mode="sample", n_samples=None, vocab_filter=None
    ):
        """
        ...
        """
        assert (
            (mode == "score" and target is not None)
            or mode == "sample"
            or mode == "encode_only"
        )
        ...
        if self.no_inputs:
            ...
            inputs_scatter = [
                [
                    Variable(
                        self._zeros(
                            max_length_inputs[i][j], batch_size, self.v_inputs[i] + 1
                        ).scatter_(2, inputs[i][j][:, :, None], 1)
                    )
        ...
        if target is not None:
            target_scatter = Variable(
                self._zeros(max_length_target, batch_size, self.v_target + 1).scatter_(
                    2, target[:, :, None], 1
                )
            )  # max_length_target * batch_size * v_target+1
        ...
        def attend(i, j, h):
            ...
            scores = (
                self.As[i - 1](
                    H[i - 1][j].view(
                        max_length_inputs[i - 1][j] * batch_size, self.hidden_size
                    ),
                    h.view(batch_size, self.hidden_size).repeat(
                        max_length_inputs[i - 1][j], 1
                    ),
                ).view(max_length_inputs[i - 1][j], batch_size)
                + attention_mask[i - 1][j]
            )
        ...
        for i in range(len(self.input_vocabularies)):
            ...
            for j in range(n_examples):
                ...
                state = self._encoder_get_init(
                    i, batch_size=batch_size, h=embeddings[i - 1][j] if i > 0 else None
                )
                hs = []
                ...
                for k in range(max_length_inputs[i][j]):
                    if i == 0:
                        ...
                        state = self.encoder_cells[i](
                            torch.cat(
                                [inputs_scatter[i][j][k, :, :], attend(i, j, h)], 1
                            ),
                            state,
                        )
                    if k + 1 < max_length_inputs[i][j]:
                        active[k + 1, :] = active[k, :] * (
                            inputs[i][j][k, :] != self.v_inputs[i]
                        )
                    h = self._cell_get_h(state)
                    hs.append(h[None, :, :])
                ...
                embedding = H[i][j].gather(
                    0,
                    Variable(
                        embedding_idx[None, :, None].repeat(1, 1, self.hidden_size)
                    ),
                )[0]
        ...
        target = (
            target
            if mode == "score"
            else self._zeros(max_length_target, batch_size).long()
        )
        if self.no_inputs:
            decoder_states = [
                self._decoder_get_init(batch_size=batch_size)
            ]  # TODO: learn seperate one of these
        else:
            decoder_states = [
                self._decoder_get_init(embeddings[self.n_encoders - 1][j])
                for j in range(n_examples)
            ]  # P
        syntax_decoder_state = self._syntax_decoder_get_init(
        ...
        for k in range(max_length_target):
            ...
            for j in range(1 if self.no_inputs else n_examples):
                ...
                p_aug = (
                    h
                    if self.no_inputs
                    else torch.cat([h, attend(self.n_encoders, j, h)], 1)
                )
            ...
            syntax_score = syntax_score + choose(
                syntax_logsoftmax, target[k, :]
            ) * Variable(active.float())
            ...
            if mode == "sample":
                target[k, :] = torch.multinomial(logsoftmax.data.exp(), 1)[:, 0]
            ...
            syntax_decoder_state = self.syntax_decoder_cell(
                target_char_scatter, syntax_decoder_state
            )  # TODO
        return target, score, syntax_score
    ...
    def beam_decode(
        self, batch_inputs=None, beam_size=None, vocab_filter=None, maxlen=None
    ):

        inputs = self._inputsToTensors(batch_inputs)

        beam = self._run_with_beam(
            inputs, beam_size=beam_size, vocab_filter=vocab_filter, maxlen=maxlen
        )
    ...
    def _run_with_beam(self, inputs, beam_size=10, vocab_filter=None, maxlen=None):
        ...
        (
            ...
            inputs, vocab_filter=vocab_filter
        )  # use hack on run
            ...
            for target, score, decoder_states, syntax_decoder_state, active in beam:
                if not any(active == True):
                    if len(new_beam) < beam_size:
                        ...
                        )  # I think it's fine not to clone here
                    ...
                    # print(k, flush=True)
                    logsoftmax = self._run_first_half(
                        k,
                        decoder_states,
                        syntax_decoder_state,
                        H,
                    ...
                    for token in list(self.target_vocabulary_index.values()) + [
                        self.v_target
                    ]:
                        # do filtering for these lines
                        candidate = self._run_second_half(
                            k,
                            logsoftmax,
                            target.clone(),
                            token,
                            score.clone(),
                            [(ds[0].clone(), ds[1].clone()) for ds in decoder_states],
                            (
                                syntax_decoder_state[0].clone(),
                                syntax_decoder_state[1].clone(),
                            ),
                            active.clone(),
                            batch_size,
                            n_examples,
                        ...
                        if len(new_beam) < beam_size:
                            ...
                            if (
                                candidate[1] > new_beam[-1][1]
                            ):  # the worst score in new_beam
    ...
    def attend_for_beam(
        self, i, j, h, H, attention_mask, max_length_inputs, batch_size
    ):
        """
        'general' attention from https://arxiv.org/pdf/1508.04025.pdf
        :param i: which encoder is doing the attending (or self.n_encoders for the decoder)
    ...
    def _run_first_half(
        ...
        for j in range(1 if self.no_inputs else n_examples):
            ...
            )  # will need H and attention_mask for attend fn
            FC.append(F.tanh(self.W(p_aug)[None, :, :]))
    ...
    def _run_second_half(
        ...
        return (
            ...
            score.clone(),
            [(ds[0].clone(), ds[1].clone()) for ds in decoder_states],
            (syntax_decoder_state[0].clone(), syntax_decoder_state[1].clone()),
            active.clone(),
        )
