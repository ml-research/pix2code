...
class Pix2Seq(nn.Module):
    ...
    def build_input_seq(self, targets, max_objects=100):
        ...
        for b_i, target in enumerate(targets):
            ...
            box_tokens = (
                (scaled_box / 1333 * self.num_bins)
                .floor()
                .long()
                .clamp(min=0, max=self.num_bins)
            )
            ...
            random_class = (
                torch.randint(0, self.num_classes, (num_noise, 1), device=device)
                + self.num_bins
                + 1
            )
            ...
            random_scaled_box = (
                torch.cat([random_box_x0y0, random_box_x1y1], dim=1) * scale_factor
            )
            random_box_tokens = (
                (random_scaled_box / 1333 * self.num_bins)
                .floor()
                .long()
                .clamp(min=0, max=self.num_bins)
            )
            ...
            if num_objects > 0:
                ...
                jitter_box = (
                    torch.rand((num_noise, 4), device=device) - 0.5
                ) * 2 * 0.2 * jitter_box_wh + jitter_box
                scaled_jitter_box = jitter_box.clamp(min=0, max=1.0) * scale_factor
                jitter_box_tokens = (
                    (scaled_jitter_box / 1333 * self.num_bins)
                    .floor()
                    .long()
                    .clamp(min=0, max=self.num_bins)
                )
...
class SetCriterion(nn.Module):
    ...
    def build_target_seq(self, targets, max_objects=100):
        ...
        for target in targets:
            ...
            fake_target_tokens = torch.zeros((num_noise, 5), dtype=torch.int64).to(
                device
            )
            ...
            target_seq = torch.cat(
                [target_tokens, end_token, fake_target_tokens], dim=0
            )
    ...
    def forward(self, outputs, targets):
        ...
        if isinstance(pred_seq_logits, list) and not self.training:
            ...
            pred_seq_logits = torch.cat(pred_seq_logits, dim=0).reshape(
                -1, self.num_vocal
            )
            target_seq = torch.cat(
        ...
        loss_seq = (
            F.cross_entropy(
                pred_seq_logits, target_seq, weight=self.empty_weight, reduction="sum"
            )
            / num_pos
        )
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        for b_i, pred_seq_logits in enumerate(out_seq_logits):
            ...
            pred_seq_logits = pred_seq_logits[: int(num_objects * 5)].reshape(
                num_objects, 5, -1
            )
            pred_boxes_logits = pred_seq_logits[:, :4, : self.num_bins + 1]
            pred_class_logits = pred_seq_logits[
                :, 4, self.num_bins + 1 : self.num_bins + 1 + self.num_classes
            ]
    ...
    @torch.no_grad()
    def get_output_seq(self, outputs, targets):
        origin_img_sizes = torch.stack([t["orig_size"] for t in targets], dim=0)
        input_img_sizes = torch.stack([t["size"] for t in targets], dim=0)
        out_seq_logits = outputs["pred_seq_logits"]
        ...
        for b_i, pred_seq_logits in enumerate(out_seq_logits):
            pred_seq_logits = pred_seq_logits.softmax(dim=-1)
            test_seq = pred_seq_logits.argmax(dim=1).tolist()
            try:
                eos = test_seq.index(2092)
            except ValueError:
                try:
                    eos = test_seq.index(2093)
                except ValueError:
                    eos = len(test_seq)
                    # raise ValueError("No end token found in the sequence")

            pred_seq_logits = pred_seq_logits[:eos]
            ...
            result = dict()
            result["scores"] = scores_per_image.cpu().numpy().tolist()
            result["labels"] = labels_per_image.cpu().numpy().tolist()
            result["boxes"] = boxes_per_image.cpu().numpy().tolist()
            results.append(result)
...
def build(args):
    ...
    num_classes = (
        20 if args.dataset_file != "coco" and args.dataset_file != "clevr" else 91  # 91
    )
    ...
    model = Pix2Seq(backbone, transformer, num_classes=num_classes, num_bins=num_bins)
