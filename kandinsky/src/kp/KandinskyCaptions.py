...
class CaptionGenerator:
    ...
    def colorShapesSize(self, kf, prefix=""):
        ...
        for s in kf:
            ...
            if addsizeattribute:
                sizestring = ""
                if s.size < smallsizecheck:
                    sizestring = "small"
                if s.size > bigsizecheck:
                    sizestring = "big"
                if len(sizestring) > 0:
                    descrition = descrition + sizestring + " "
    ...
    def _getMaxShapesAndColorsPatterns(self, kf):
        ...
        if maxn > 0:
            ...
            if colordominance:
                ...
                for s in kf:
                    if s.shape != maxshape or s.color != maxcolor:
                        kfnew.append(s)
            ...
            descrition = (
                inflectEng.number_to_words(maxn) + " " + maxcolor + " " + maxshape
            )
            if maxn > 1:
