...
def compute_best_valid(job, metric, greater_is_better):
    ...
    best_checkpoint_idx = result_table.sort_values(
        "value", ascending=greater_is_better
    )["step"].iloc[-1]
    best_checkpoint = os.path.join(job, f"checkpoint_{best_checkpoint_idx}.pth")
    target_checkpoint_path = os.path.join(job, f"checkpoint_best_{metric_to_print}.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="If greater is better for the chosen val metric," "1 if true, 0 if false.",
    )
