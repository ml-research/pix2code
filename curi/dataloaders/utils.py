...
def _numeric_string_array_to_numbers(numeric_string_array, cast_type="float"):
    ...
    for t in numeric_string_array:
        numeric_array.append(np.array([cast_fn(x) for x in t.split(",")]))
...
def make_adhoc_dataset_with_buffer(
    dir, extensions=None, is_valid_file=None, buffer_threshold=100000
):
    ...
    ret = os.system("find %s > %s" % (dir, tfile))
    if ret == 0:
...
class DatasetFolderPathIndexing(VisionDataset):
    def __init__(
        ...
        super(DatasetFolderPathIndexing, self).__init__(
            root, transform=transform, target_transform=target_transform
        )
        samples = make_adhoc_dataset_with_buffer(self.root, extensions, is_valid_file)
        if len(samples) == 0:
            raise (
                RuntimeError(
    ...
    def __getitem__(self, index):
        ...
        # remove annotations from kandinsky dataset if data type is schema
