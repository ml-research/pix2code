...
class TraditionalDataset(data.Dataset):
    def __init__(
        ...
        with open(os.path.join(root_dir, image_hypothesis_mapping_file), "rb") as f:
            datum_hypothesis_data = pickle.load(f)
            all_data_to_hypotheses = datum_hypothesis_data["images_to_hypotheses"]
            all_data_to_labels = datum_hypothesis_data["images_to_labels"]

            if len(dataset.hypothesis) != _SPLITS_TO_NUM_HYPOTHESES[split_name]:
                raise ValueError(
                    "Expected %d hypotheses", _SPLITS_TO_NUM_HYPOTHESES[split_name]
                )
        ...
        if self._split_name is not None:
            ...
            for hyp_idx, _ in enumerate(dataset.hypothesis):
                this_images = np.random.choice(
                    dataset.image_id_list[hyp_idx],
    ...
    def __getitem__(self, index):
        ...
        labels = self._all_data_to_labels[self.this_dataset_datapoint_subset[index]]
...
class MetaDataset(data.Dataset):
    def __init__(
        ...
        hypotheses_in_split = set([x["support"].hypothesis for x in self._meta_dataset])
        ...
        self._all_hyp_str_to_idx = {
            v: k for k, v in enumerate(self._all_hypotheses_across_splits)
        }
    ...
    def __getitem__(self, idx):
        ...
        hypothesis_encoded = self._vocabulary.encode_string(hypothesis_string).long()
        ...
        for episode_split in ["support", "query"]:
            ...
            labels = np.array(self._meta_dataset[idx][episode_split].data_labels)

            data_in_split = [x["datum"] for x in self._loader.get_item_list(data_list)]
            ...
            if episode_split == "support":
                ...
                for valid_hyp, valid_hyp_log_prob in zip(
                    self._meta_dataset[idx][episode_split].all_valid_hypotheses,
                    self._meta_dataset[idx][episode_split].posterior_logprobs,
                ):
                    all_consistent_hypotheses_idx_sparse[
                        self._all_hyp_str_to_idx[valid_hyp]
                    ] = True

                    posterior_probs_sparse[
                        self._all_hyp_str_to_idx[valid_hyp]
                    ] = np.exp(valid_hyp_log_prob)
            ...
            if episode_split == "query":
                optimistic_query_labels = np.array(
                    self._meta_dataset[idx][episode_split].optimistic_data_labels
                )
...
def get_adhoc_loader(cfg, batch_size, splits):
    ...
    for this_split in splits:
        ...
        path_to_hypothesis_dataset = os.path.join(
            HYPOTHESIS_DATASET, cfg.get(this_split)
        )
        ...
        if this_split != "cross_split":
            this_dataset = MetaDataset(
                ...
                load_multihot_labels_per_datapoint=False,  # TODO: cfg._mode != "train"
