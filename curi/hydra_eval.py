...
class _Workplace(object):
    def __init__(self, cfg):
        ...
        logging.warn(
            "Please ensure train, val and test " "loaders give the same vocabulary."
        )

        # Initialize the model/trainer
        model = load_model(self.cfg, self._vocabulary)
        loss_fn = hydra.utils.instantiate(
            config=self.cfg.loss, pad_token_idx=self._vocabulary.pad()
        )
        ...
        if self.cfg.model_or_oracle_metrics == "oracle":
            ...
            if (
                self.cfg.eval_cfg.best_test_metric == ""
                or self.cfg.eval_cfg.best_test_metric == None
            ):
                self._best_test_metric_or_oracle = ""
            else:
                self._best_test_metric_or_oracle = (
                    self.cfg.eval_cfg.best_test_metric.replace("/", "_")
                )
        ...
        if self._eval_split_name == "test":
            ...
            write_metrics_file = "test_metrics.txt"
        else:
            write_metrics_file = (
                self.cfg.get(self._eval_split_name).rstrip(".pkl") + "_metrics.txt"
            )
    ...
    def run_eval(self):
        rtpt = RTPT(
            name_initials="XX",
            experiment_name="CURI-Eval",
            max_iterations=self.cfg.opt.max_steps * 8,
        )
        rtpt.start()
        ...
        if self.cfg.model_or_oracle_metrics == "oracle":
            ...
            for eval_object in ["weak_oracle", "random", "oracle"]:
                all_oracle_baselines[
                    "map_" + eval_object
                ] = self._costly_evaluator.evaluate(
                    eval_object=eval_object,
                    num_batches=_NUM_EVAL_BATCHES_FOR_SPLIT[self._eval_split_name],
                )
        ...
        if (
            self.cfg.model_or_oracle_metrics == "model"
            and self.cfg.eval_cfg.evaluate_all == True
        ):
            ...
            if self.cfg.eval_cfg.evaluate_once == True:
                raise ValueError(
                    "Evaluate once and evaluate all cannot be true at once."
                )
        ...
        while True:
            if self.cfg.model_or_oracle_metrics == "model":
                if self.cfg.eval_cfg.evaluate_all == True:
                    (
                        active_checkpoint,
                        active_iteration,
                    ) = all_checkpoint_paths_and_idx.pop()
                else:
                    # Test set is always evaluated with the best checkpoint from validation.
                    if self._eval_split_name == "testt": # TODO: fix this
                        (
                            active_checkpoint,
                            active_iteration,
            ...
            if active_iteration is None or active_iteration != current_iteration:
                ...
                is_repeated_checkpoint = self._trainer.write_metrics(
                    all_metrics,
                ...
                if (
                    ...
                    and self.cfg.eval_cfg.evaluate_all == True
                    and (self.cfg.eval_cfg.sort_validation_checkpoints == True)
                ):
                    logging.info(
                        "Reached a checkpoint that has already been "
            ...
            if (
                self.cfg.model_or_oracle_metrics == "oracle"
                or self.cfg.eval_cfg.evaluate_once == True
                or self._eval_split_name == "test"
            ):
            ...
            if self.cfg.eval_cfg.evaluate_all == True:
                if len(all_checkpoint_paths_and_idx) == 0:
                    logging.info(
                        "No checkpoints left to evaluate. Finished evaluation."
                    )
                    break
